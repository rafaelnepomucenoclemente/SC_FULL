...
MAPTARENA CTChart::m_mapTArena;
MAPTPORTALLINK CTChart::m_mapTPORTALLINK;
...
void CTChart::InitTDESTINATION( CString strPath)
{
	CFile file( LPCSTR(strPath), CFile::modeRead|CFile::typeBinary);
	CArchive ar( &file, CArchive::load);
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		ar	>> wPortalID
			...
			>> bCanUse[TCONTRY_C]
			>> bCanUse[TCONTRY_B]
			>> bCanUse[TCONTRY_PEACE]
			>> wMapID
			>> bUnitX
			>> bUnitZ;

		LPTPORTALLINK pTUNIT = FindTPORTALLINK(
			wMapID,
			bUnitX,
			bUnitZ);
		...
		if(pTUNIT)
		{
			LPTPORTAL pTPORTAL = GetTPORTAL(wDestID);

			if(pTPORTAL)
			{
				LPVTNPCPORTAL pTDATA = bIsLocal ? pTUNIT->m_vTLOCAL : bIsBase ? pTUNIT->m_vTBASEPORTAL : pTUNIT->m_vTFROMPORTAL;

				if(pTDATA)
					for( int j=0; j<TCONTRY_COUNT; j++)
						if(bCanUse[j])
						{
							LPTNPCPORTAL pTNPCPORTAL = new TNPCPORTAL();

							pTNPCPORTAL->m_pTPORTAL = pTPORTAL;
							pTNPCPORTAL->m_wNpcID = wNpcID;

							if(bIsHub)
								pTUNIT->m_vTHUB[j] = pTNPCPORTAL;

							pTDATA[j].push_back(pTNPCPORTAL);
						}
			}
		}
	}
...
void CTChart::InitTUNITLINK( CString strPath)
{
	ReleaseTPORTALLINK();
	...
	for( WORD i=0; i<wCount; i++)
	{
		...
		if(!pTUNIT)
		{
			pTUNIT = new TPORTALLINK();

			pTUNIT->m_vID.m_vID.m_wMapID = wMapID;
			pTUNIT->m_vID.m_vID.m_bUnitX = bUnitX;
			pTUNIT->m_vID.m_vID.m_bUnitZ = bUnitZ;

			m_mapTPORTALLINK.insert( MAPTPORTALLINK::value_type(
				pTUNIT->m_vID.m_dwID,
				pTUNIT));
		}
		...
		LPTPORTALLINK pTOBJ = FindTPORTALLINK(
			wMapID,
			bUnitX,
			bUnitZ);
		...
		if(!pTOBJ)
		{
			pTOBJ = new TPORTALLINK();

			pTOBJ->m_vID.m_vID.m_wMapID = wMapID;
			pTOBJ->m_vID.m_vID.m_bUnitX = bUnitX;
			pTOBJ->m_vID.m_vID.m_bUnitZ = bUnitZ;
		...
		switch(bType)
		{
		case TPORTALLINK_LINK	: pTUNIT->m_vTLINK[bCountryID].push_back(pTOBJ); break;
		case TPORTALLINK_BASE	: pTUNIT->m_vTBASE[bCountryID].push_back(pTOBJ); break;
		}
	}
}

LPTPORTALLINK CTChart::FindTPORTALLINK( WORD wMapID,
									    BYTE bUnitX,
										BYTE bUnitZ)
{
	return FindTPORTALLINK( MAKELONG(
		wMapID,
		MAKEWORD(
		bUnitX,
		bUnitZ)));
...
LPTPORTALLINK CTChart::FindTPORTALLINK( DWORD dwID)
{
	MAPTPORTALLINK::iterator finder = m_mapTPORTALLINK.find(dwID);

	if( finder != m_mapTPORTALLINK.end() )
		return (*finder).second;

	return NULL;
}

void CTChart::ReleaseTPORTALLINK()
{
	MAPTPORTALLINK::iterator itTUNIT;

	for( itTUNIT = m_mapTPORTALLINK.begin(); itTUNIT != m_mapTPORTALLINK.end(); itTUNIT++)
		delete (*itTUNIT).second;

	m_mapTPORTALLINK.clear();
}
...
void CTChart::InitTQUESTPOS( CString strPath)
{
	CFile file( LPCSTR(strPath), CFile::modeRead|CFile::typeBinary);
	CArchive ar( &file, CArchive::load);
	...
	for( DWORD i=0; i<dwCount; i++)
	{
		...
		LPTQUEST pTQUEST = FindTQUEST(dwQuestID);
		for( DWORD j=0; j<dwTermCount; j++)
		{
			LPTTERMPOS pTTERM = new TTERMPOS();
			...
			if(pTQUEST)
				pTQUEST->m_vTTERMPOS.push_back(pTTERM);
			else
				delete pTTERM;
		}
	}
...
void CTChart::InitTArena( CString strPath )
{
	ReleaseTArena();
	...
	for( ; i < wCount; ++i )
	{
		LPTARENA pTArena = new TARENA;

		ar >> (pTArena->wArena)
			>> (pTArena->bType);
			//>> (pTArena->m_dwFee);

		pTArena->m_dwFee = 300000;

		m_mapTArena.insert( MAPTARENA::value_type(
			pTArena->wArena,
			pTArena) );
	}
}

void CTChart::ReleaseTArena()
{
	MAPTARENA::iterator it, end;
	it = m_mapTArena.begin();
	end = m_mapTArena.end();
	for(; it != end ; ++it )
	{
		if( it->second )
...
LPTARENA CTChart::FindTArena( WORD wArena )
{
	MAPTARENA::iterator find = m_mapTArena.find( wArena );

	if ( find == m_mapTArena.end() )
		return NULL;

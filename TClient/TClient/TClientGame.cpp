...
FLOAT g_fWaterHeight = 0.0f;
BOOL CTClientGame::g_TestEnableFOG = TRUE;
...
{
public:
	CExceptCol( CTClientObjBase *pTOBJ)
	{
		m_pTOBJ = pTOBJ;

		if(m_pTOBJ)
		{
			m_bCollitionType = m_pTOBJ->m_bCollisionType;
			m_pTOBJ->m_bCollisionType = TCOLLISION_NONE;
		}
	};
	...
	{
		if(m_pTOBJ)
			m_pTOBJ->m_bCollisionType = m_bCollitionType;
	};

protected:
	CTClientObjBase *m_pTOBJ;
	BYTE m_bCollitionType;
};
...
CTClientGame::CTClientGame()
{
	...
	m_bChallengeGame = FALSE;
	m_bChallengeStatus = BS_NOBATTLE;
	m_bPrevChallengeStatus = BS_NOBATTLE;
	...
	ZeroMemory( m_vTNODEPATH, (TNODEGUIDE_COUNT + 1) * sizeof(D3DXVECTOR3));
	ZeroMemory( &m_vTAUTOPATH, sizeof(D3DXVECTOR3));
	ZeroMemory( &m_vTAUTOPREV, sizeof(D3DXVECTOR3));
	ZeroMemory( &m_vTAUTOTARGET, sizeof(TTERMPOS));
	ZeroMemory( &m_vTAUTOPLANE, sizeof(D3DXPLANE));
...
void CTClientGame::GetSecondToRemainTime(
	DWORD dwSecond,
	DWORD* dwDay,
	DWORD* dwHour,
	DWORD* dwMin,
...
void CTClientGame::CalcSCENE( DWORD dwTick)
{
	...
	if( m_dwRemainTickVerifyMaintain == 0 )
	{
		if( !m_pMainChar->VerifySkillFunc() && GetSession() )
			GetSession()->SendCS_ACDCLOSE_REQ( 2 );
	...
	m_dwRemainSecondChangeAID -= min(m_dwRemainSecondChangeAID, dwTick);
}
...
HRESULT CTClientGame::Render( DWORD dwTickCount)
{
	...
	if(m_bShowDebugInfo)
	{
		CString strPOS;
		strPOS.Format(
			"X = %.2f, Y = %.2f, Height = %.2f, World(%.2f, %.2f, %.2f), Dir = %d, Region = %d TDBB Level = %d\n"
			"Camera (Dir:%d, Pitch:%d, Dist:%.2f, Pos(%.2f,%.2f,%.2f), Target(%.2f,%.2f,%.2f))\n"
			...
			m_pCamera->m_vPosition.x, m_pCamera->m_vPosition.y, m_pCamera->m_vPosition.z,
			m_pCamera->m_vTarget.x, m_pCamera->m_vTarget.y, m_pCamera->m_vTarget.z, 
		...
		m_FontPOS.TextOut( m_pDevice->m_pDevice, strPOS, 0, 600);
		...
		D3DXMATRIX vTMAT;
		D3DXMatrixIdentity(&vTMAT);
		m_pCamera->Activate(TRUE);

		if( m_vMAP.m_pMAP && m_vMAP.m_vTNODE[m_vMAP.m_pMAP->m_bUNIT[4]].m_fLengthX > 0.0f )
		{
			CTClientPath *pTPATH = &m_vMAP.m_vTNODE[m_vMAP.m_pMAP->m_bUNIT[4]];
			...
			for( itTPATH = pTPATH->m_mapTPATH.begin(); itTPATH != pTPATH->m_mapTPATH.end(); itTPATH++)
			{
				...
				if(pTPATH->FindT3DPOINT( &vTPATH, (*itTPATH).first))
				{
					D3DXVECTOR3 vTDIR = vTPATH - vTPOS;
					vTDIR.y = 0.0f;

					if( D3DXVec3Length(&vTDIR) < 64.0f )
					{
						D3DVECTOR vTVERT[2];

						vTPATH.y = vTPOS.y + m_pMainChar->m_fSizeY / 2.0f;
						vTPATH.y = m_vMAP.GetHeight(
							m_pMainChar,
							NULL,
							&vTPATH,
							0.0f,
						...
						m_pDevice->m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, 0xFF00F0FF);
						m_pDevice->m_pDevice->SetRenderState( D3DRS_FILLMODE, D3DFILL_SOLID);
						m_pDevice->m_pDevice->SetRenderState( D3DRS_ZWRITEENABLE, TRUE);
						m_pDevice->m_pDevice->SetRenderState( D3DRS_ZENABLE, TRUE);
						m_pDevice->m_pDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE);
						m_pDevice->m_pDevice->SetTransform( D3DTS_WORLD, &vTMAT);
						m_pDevice->m_pDevice->SetFVF(D3DFVF_XYZ);

						m_pDevice->m_pDevice->SetTexture( 0, NULL);
						m_pDevice->m_pDevice->DrawPrimitiveUP(
							D3DPT_LINELIST, 1,
							vTVERT, sizeof(D3DVECTOR));

						CString strLineID;
						strLineID.Format( "%d", (*itTPATH).first);
						vTPATH.y += 2.0f;
		...
		if(m_pTAUTOPATH)
		{
			D3DXVECTOR3 vTPATH = m_vTAUTOPATH;
			D3DVECTOR vTVERT[2];

			vTPATH.y = m_pMainChar->GetPositionY() + m_pMainChar->m_fSizeY / 2.0f;
			vTPATH.y = m_vMAP.GetHeight(
				m_pMainChar,
				NULL,
				&vTPATH,
			...
			m_pDevice->m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, 0xFFFFF000);
			m_pDevice->m_pDevice->SetRenderState( D3DRS_FILLMODE, D3DFILL_SOLID);
			m_pDevice->m_pDevice->SetRenderState( D3DRS_ZWRITEENABLE, TRUE);
			m_pDevice->m_pDevice->SetRenderState( D3DRS_ZENABLE, TRUE);
			m_pDevice->m_pDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE);
...
void CTClientGame::ResetTFOG( BYTE bWATER)
{
	...
	if( CTClientGame::g_TestEnableFOG )
		m_pDevice->m_pDevice->SetRenderState( D3DRS_FOGENABLE, TRUE );
	else
		m_pDevice->m_pDevice->SetRenderState( D3DRS_FOGENABLE, FALSE );
...
void CTClientGame::ResetVisibleOBJ( DWORD dwTick)
{
	...
	if( pTPANNEL && pTPANNEL->m_bShowPath && pTPANNEL->m_dwCurQuestID )
		for( i=TNODEGUIDE_COUNT - 1; i>=0; i--)
			if(m_vTNODEGUIDE[i].m_bVisible)
				PushDrawOBJ(&m_vTNODEGUIDE[i]);
...
{
	if( pTOBJ->m_bGMHide && pTOBJ->m_dwID != m_pMainChar->m_dwID  )
	{
...
void CTClientGame::RenderTSILHOUETTE()
{
	...
	vTCAM.InitCamera(
		...
		(FLOAT) m_vMAP.m_pMAP->m_nTileLength * m_vMAP.m_pMAP->m_nUnitLength,
		m_pCamera->m_fFOV,
	...
	CTachyonMesh::BeginGlobalDraw(m_pDevice->m_pDevice);
	CTachyonMesh::SetGlobalZState(FALSE, FALSE);
	...
	CTachyonMesh::SetGlobalZState(TRUE, TRUE);
							   ...
							   BYTE bDrawGROUND)
{
	...
	if( m_pTNODEPATH || m_pTNODEPORTAL )
	{
		fDepthBias = TOBJSELECT_BIAS;
		m_pDevice->m_pDevice->SetRenderState( D3DRS_DEPTHBIAS, *((LPDWORD) &fDepthBias));
		m_pDevice->m_pDevice->SetTexture( 0, m_pTGROUNDTEX->GetTexture());
		m_pDevice->m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, 0xFFFFFF00);

		vKEY = m_pTGROUNDTEX->GetUV();
		CTMath::GetUVMatrix( &vUV, &vKEY);

		for( itTOBJ = pLIST->begin(); itTOBJ != pLIST->end(); itTOBJ++)
			if((*itTOBJ)->GetDrawNodeMark( m_pTNODEPATH, m_pTNODEPORTAL))
				RenderTSQUARE( (*itTOBJ), &vUV, NULL, TNODEMARK_SCALE);
	}
...
BYTE CTClientGame::RenderTICON( CTClientObjBase *pTOBJ, const D3DXVECTOR3 &vPROJ, FLOAT fBaseHeight, FLOAT fLEFT)
{
	...
	vTICON._41 = (FLOAT) INT(vTICON._41) - 0.5f;
	vTICON._42 = (FLOAT) INT(vTICON._42) - 0.5f;
...
BYTE CTClientGame::FixCamera( DWORD dwTick)
{
	...
	if( m_bMoveType != TMAINMOVE_NORMAL && !m_pTAUTOPATH )
		return FALSE;
...
D3DXVECTOR2 CTClientGame::GetTNODEGOAL( LPTTERMPOS pTNODEPATH,
									    LPTNPCPORTAL pTNODEPORTAL)
{
	if(pTNODEPORTAL)
	{
		LPTNPC pTNPC = m_vMAP.GetTNPC(pTNODEPORTAL->m_wNpcID);

		if(pTNPC)
			return D3DXVECTOR2( pTNPC->m_fPosX, pTNPC->m_fPosZ);
	}

	return D3DXVECTOR2( pTNODEPATH->m_fPosX, pTNODEPATH->m_fPosZ);
}

BYTE CTClientGame::GetTNODELOCALPORTAL( LPTNPCPORTAL *pTRESULT,
										...
										int nCashIndex)
{
	if( !pTRESULT || !pTPATH || !pFROM || !pTO || !m_vMAP.m_pMAP )
		return FALSE;
	int nLength = m_vMAP.m_pMAP->m_nUnitLength * m_vMAP.m_pMAP->m_nTileLength;

	BYTE bFromX = BYTE(INT(pFROM->x) / nLength);
	BYTE bFromZ = BYTE(INT(pFROM->z) / nLength);

	BYTE bToX = BYTE(INT(pTO->x) / nLength);
	BYTE bToZ = BYTE(INT(pTO->z) / nLength);

	if( bFromX != bToX || bFromZ != bToZ )
		return FALSE;

	LPTPORTALLINK pTUNIT = CTChart::FindTPORTALLINK(
		m_vMAP.m_wMapID,
		bFromX,
		bFromZ);
	BYTE bCountryID = m_pMainChar->IsDisguise() ? m_pMainChar->m_bDisguise : m_pMainChar->GetWarCountry();

	if(!pTUNIT)
		return FALSE;
	pTPATH->BuildTSTARTPOINT( pFROM, TPATH_CASH_TEMP);

	for( int i=0; i<INT(pTUNIT->m_vTLOCAL[bCountryID].size()); i++)
	{
		LPTNPCPORTAL pTPORTAL = pTUNIT->m_vTLOCAL[bCountryID][i];

		if( pTPORTAL && pTPORTAL->m_pTPORTAL && pTPORTAL->m_wNpcID )
		{
			LPTNPC pTNPC = m_vMAP.GetTNPC(pTPORTAL->m_wNpcID);

			if(pTNPC)
			{
				LPTPATHINFO pTFROMPATH = NULL;
				LPTPATHINFO pTTOPATH = NULL;

				D3DXVECTOR3 vTPOS(
					pTPORTAL->m_pTPORTAL->m_fPosX,
					pTO->y,
					pTPORTAL->m_pTPORTAL->m_fPosZ);

				if(pTPATH->FindTPATH( &pTTOPATH, pTO, &vTPOS, nCashIndex))
				{
					vTPOS.x = pTNPC->m_fPosX;
					vTPOS.y = pTNPC->m_fPosY;
					vTPOS.z = pTNPC->m_fPosZ;

					if(pTPATH->FindTPATH( &pTFROMPATH, pFROM, &vTPOS, TPATH_CASH_TEMP))
					{
						if(pTFROMPATH)
							if(!pTFROMPATH->m_vTPATH.empty())
							{
								MAPTPATHDATA::iterator itTPATH = pTPATH->m_mapTPATH.find(pTFROMPATH->m_vTPATH.back());

								if( itTPATH != pTPATH->m_mapTPATH.end() )
								{
									LPMAPTPATHINFO pTLOCAL = (*itTPATH).second;

									if(pTLOCAL)
									{
										MAPTPATHINFO::iterator itTINFO = pTLOCAL->find(pTFROMPATH->m_vTPATH.front());

										if( itTINFO != pTLOCAL->end() )
											pTFROMPATH = (*itTINFO).second;
										else
											pTFROMPATH = NULL;
									}
									else
...
BYTE CTClientGame::GetTNODEPORTAL( LPTNPCPORTAL *pTRESULT,
								   LPD3DXVECTOR3 pFROM,
								   LPTTERMPOS pTO)
{
	if( !pTRESULT || !pFROM || !pTO || !m_vMAP.m_pMAP )
		return FALSE;
	int nLength = m_vMAP.m_pMAP->m_nUnitLength * m_vMAP.m_pMAP->m_nTileLength;
	...
	BYTE bToX = BYTE(INT(pTO->m_fPosX) / nLength);
	BYTE bToZ = BYTE(INT(pTO->m_fPosZ) / nLength);

	if( m_vMAP.m_wMapID == pTO->m_dwMapID && bFromX == bToX && bFromZ == bToZ )
	{
		(*pTRESULT) = NULL;
		return TRUE;
	}
	...
	LPTNPCPORTAL pTPORTAL = FindTNODEPORTAL(
		&pTUNIT->m_vTFROMPORTAL[bCountryID],
		pFROM,
		pTO);
	FLOAT fDIST = 0.0f;
	...
	for( int i=0; i<INT(pTUNIT->m_vTLINK[bCountryID].size()); i++)
	{
		LPTPORTALLINK pTLINK = pTUNIT->m_vTLINK[bCountryID][i];

		if(pTLINK)
		{
			LPTNPCPORTAL pTLOCAL = FindTNODEPORTAL(
				&pTLINK->m_vTFROMPORTAL[bCountryID],
				pFROM,
				NULL);

			if(pTLOCAL)
			{
				LPTNPC pTNPC = m_vMAP.GetTNPC(pTLOCAL->m_wNpcID);
	...
	pTUNIT = CTChart::FindTPORTALLINK(
		m_vMAP.m_wMapID,
		bFromX,
		bFromZ);
	...
	pTPORTAL = FindTNODEPORTAL(
		&pTUNIT->m_vTBASEPORTAL[bCountryID],
		pFROM,
		NULL);
	...
	for( i=0; i<INT(pTUNIT->m_vTLINK[bCountryID].size()); i++)
	{
		LPTPORTALLINK pTLINK = pTUNIT->m_vTLINK[bCountryID][i];
...
LPTNPCPORTAL CTClientGame::FindTNODEPORTAL( LPVTNPCPORTAL pTPORTAL,
										    LPD3DXVECTOR3 pFROM,
											LPTTERMPOS pTO)
{
	if( !pTPORTAL || !pFROM || !m_vMAP.m_pMAP )
		return NULL;
	int nLength = m_vMAP.m_pMAP->m_nUnitLength * m_vMAP.m_pMAP->m_nTileLength;
	...
	LPTNPCPORTAL pTRESULT = NULL;
	FLOAT fDIST = 0.0f;

	for( int i=0; i<INT(pTPORTAL->size()); i++)
	{
		LPTNPCPORTAL pTLOCAL = (*pTPORTAL)[i];

		if( pTLOCAL && pTLOCAL->m_pTPORTAL )
		{
			LPTNPC pTNPC = m_vMAP.GetTNPC(pTLOCAL->m_wNpcID);

			if(pTNPC)
			{
				BYTE bNpcX = BYTE(INT(pTNPC->m_fPosX) / nLength);
				BYTE bNpcZ = BYTE(INT(pTNPC->m_fPosZ) / nLength);
...
BYTE CTClientGame::BuildTNODEPATH( CTClientObjBase *pTOBJ,
								   LPTTERMPOS pTNODETARGET,
								   LPTNPCPORTAL *pTNODEPORTAL,
								   LPD3DXVECTOR3 pTNODEPATH,
								   FLOAT *pVDIST,
								   int nNodeCount,
								   int nCashIndex,
								   BYTE bReset)
{
	if( !pTOBJ || !pTNODETARGET || !pTNODEPORTAL ||
		!pTNODEPATH || !pVDIST || !m_vMAP.m_pMAP ||
		nCashIndex < 0 || nCashIndex >= TPATH_CASH_COUNT )
		return FALSE;

	D3DXMATRIX vTOBJPOS = pTOBJ->GetPositionMat();
	D3DXVECTOR3 vTPOS(
		vTOBJPOS._41,
		vTOBJPOS._42,
		vTOBJPOS._43);
	...
	D3DXVECTOR3 vTDEST(
		pTNODETARGET->m_fPosX,
		pTNODETARGET->m_fPosY,
		pTNODETARGET->m_fPosZ);

	CTClientObjBase *pTEXCEPT = NULL;
	BYTE bLocalPortal = FALSE;
	(*pTNODEPORTAL) = NULL;

	if( GetTNODEPORTAL( pTNODEPORTAL, &vTPOS, pTNODETARGET) && (*pTNODEPORTAL) )
	{
		LPTNPC pTNPC = m_vMAP.GetTNPC((*pTNODEPORTAL)->m_wNpcID);
		...
		vTDEST.x = pTNPC->m_fPosX;
		vTDEST.y = pTNPC->m_fPosY;
		vTDEST.z = pTNPC->m_fPosZ;

		pTEXCEPT = m_vMAP.GetTNPCINST((*pTNODEPORTAL)->m_wNpcID);
	}

	CExceptCol vExceptColGPortal(pTEXCEPT);
	pTEXCEPT = NULL;

	D3DXVECTOR3 vTSTART = vTPOS;
	D3DXVECTOR3 vTLAST = vTDEST;

	D3DXVECTOR3 vTDIR = vTLAST - vTPOS;
	vTDIR.y = 0.0f;

	if( D3DXVec3Length(&vTDIR) < TTOTAL_NODE_LENGTH && m_vMAP.CanMove( m_pMainChar, &vTPOS, &vTLAST) )
	{
	...
	CTClientPath *pTPATH = NULL;
	LPTPATHINFO pTINFO = NULL;

	int nSIZE = m_vMAP.m_pMAP->m_nUnitLength * m_vMAP.m_pMAP->m_nTileLength;
	int nUnitX = INT(vTPOS.x) / nSIZE - m_vMAP.m_pMAP->m_nUnitX;
	int nUnitZ = INT(vTPOS.z) / nSIZE - m_vMAP.m_pMAP->m_nUnitZ;

	if( nUnitX >= 0 && nUnitX < 3 && nUnitZ >= 0 && nUnitZ < 3 )
	{
		pTPATH = &m_vMAP.m_vTNODE[m_vMAP.m_pMAP->m_bUNIT[nUnitZ * 3 + nUnitX]];

		if(pTPATH)
		{
			if(bReset)
				pTPATH->BuildTSTARTPOINT( &vTLAST, nCashIndex);

			if(!pTPATH->FindTPATH( &pTINFO, &vTLAST, &vTPOS, nCashIndex))
				if( GetTNODELOCALPORTAL( pTNODEPORTAL, &pTINFO, pTPATH, &vTPOS, &vTLAST, nCashIndex) && (*pTNODEPORTAL) )
				{
					LPTNPC pTNPC = m_vMAP.GetTNPC((*pTNODEPORTAL)->m_wNpcID);
	...
	CExceptCol vExceptColLPortal(pTEXCEPT);
	pTEXCEPT = NULL;
	...
	while( nIndex < nNodeCount )
	{
		int nCount = pTINFO ? INT(pTINFO->m_vTPATH.size()) : 0;

		D3DXVECTOR3 vTPATH;
		D3DXVECTOR3 vTGOAL;

		if( !pTPATH || !pTINFO || nPath >= nCount || !pTPATH->FindT3DPOINT( &vTPATH, pTINFO->m_vTPATH[nCount - nPath - 1]) )
		{
			vTPATH.x = vTDEST.x;
			vTPATH.y = 0.0f;
			vTPATH.z = vTDEST.z;
		...
		vTPATH.y = vTPOS.y + pTOBJ->m_fSizeY / 2.0f;
		vTGOAL = vTPATH - vTSTART;
		vTGOAL.y = 0.0f;

		FLOAT fTOTAL = D3DXVec3Length(&vTGOAL);
		if( fTOTAL > TTOTAL_NODE_LENGTH - fMOVE )
		{
			vTGOAL = vTSTART + (TTOTAL_NODE_LENGTH - fMOVE) * vTGOAL / fTOTAL;
			vTGOAL.y = vTPATH.y;
		...
		vTDIR = vTGOAL - vTSTART;
		vTDIR.y = 0.0f;
		fTOTAL = fMOVE + D3DXVec3Length(&vTDIR);

		while( nIndex < nNodeCount && (fDIST + 0.01f <= fTOTAL || (
			fabs(vTGOAL.x - vTLAST.x) < 0.01f &&
			fabs(vTGOAL.z - vTLAST.z) < 0.01f)) )
		{
			pTOBJ->SetPositionX(vTSTART.x);
			pTOBJ->SetPositionY(vTSTART.y);
			pTOBJ->SetPositionZ(vTSTART.z);

			D3DXVECTOR3 vTARGET = m_vMAP.GetTargetPoint(
				pTOBJ,
				&vTGOAL);

			vTDIR = vTARGET - vTSTART;
			vTDIR.y = 0.0f;
			...
			if( fLength < 0.01f )
			{
				...
				fLength = D3DXVec3Length(&vTDIR);
			}

			vTSTART = vTARGET;
			fMOVE += fLength;

			vTGOAL.y = vTARGET.y + pTOBJ->m_fSizeY / 2.0f;
			vTDIR = vTGOAL - vTSTART;
			vTDIR.y = 0.0f;
			fTOTAL = fMOVE + D3DXVec3Length(&vTDIR);

			while( nIndex < nNodeCount && (fDIST + 0.01f <= fMOVE || (
				fabs(vTARGET.x - vTLAST.x) < 0.01f &&
				fabs(vTARGET.z - vTLAST.z) < 0.01f)) )
			{
				pTNODEPATH[nIndex] = vTARGET;
				fDIST += pVDIST[nIndex];
				nIndex++;
			}
		}
	}
	pTOBJ->SetPosition(vTOBJPOS);

	return TRUE;
}

BYTE CTClientGame::ResetTPATHDATA()
{
	CTGaugePannel *pTPANNEL = (CTGaugePannel *) m_vTFRAME[TFRAME_GAUGE];
	LPTTERMPOS pTNODEPATH = NULL;

	if( pTPANNEL && pTPANNEL->m_dwCurQuestID )
	{
		CTClientQuest *pTQUEST = m_pMainChar->FindTQuest(pTPANNEL->m_dwCurQuestID);

		if( pTQUEST && pTQUEST->m_pTQUEST )
		{
			for( int i=0; i<INT(pTQUEST->m_pTQUEST->m_vTTERM.size()); i++)
			{
				LPTTERM pTTERM = pTQUEST->m_pTQUEST->m_vTTERM[i];

				if( pTTERM && pTTERM->m_bType != QTT_TIMER )
				{
					CTClientTerm *pTerm = pTQUEST->FindTTerm(
						pTTERM->m_dwID,
						pTTERM->m_bType);

					if( !pTerm || pTerm->GetResult() == TTERMRESULT_INCOMPLETE )
					{
						for( int j=0; j<INT(pTQUEST->m_pTQUEST->m_vTTERMPOS.size()); j++)
							if( pTQUEST->m_pTQUEST->m_vTTERMPOS[j]->m_bType == pTTERM->m_bType &&
								pTQUEST->m_pTQUEST->m_vTTERMPOS[j]->m_dwID == pTTERM->m_dwID )
							{
								pTNODEPATH = pTQUEST->m_pTQUEST->m_vTTERMPOS[j];
								break;
							}
			...
			if(!pTNODEPATH)
				for( i=0; i<INT(pTQUEST->m_pTQUEST->m_vTTERMPOS.size()); i++)
					if( pTQUEST->m_pTQUEST->m_vTTERMPOS[i]->m_bType == QTT_TCOMP_POS )
					{
						pTNODEPATH = pTQUEST->m_pTQUEST->m_vTTERMPOS[i];
						break;
					}
		}
		else
		{
			pTPANNEL->m_bShowPath = FALSE;
			pTPANNEL->m_bAutoPath = FALSE;
			pTPANNEL->m_dwCurQuestID = 0;
		}
	}

	FLOAT vDIST[TNODEGUIDE_COUNT + 1] = {
		TCHAR_NODE_LENGTH,
		TGUIDE_NODE_LENGTH,
		TGUIDE_NODE_LENGTH,
		0.0f};
	...
	if( pTNODEPATH && pTNODEPATH->m_dwNpcID )
		pTEXCEPT = m_vMAP.GetTNPCINST(pTNODEPATH->m_dwNpcID);

	CExceptCol vExceptCol_pTNODEPATH(pTEXCEPT);
	pTEXCEPT = NULL;

	if( m_pTNODEPATH && m_pTNODEPATH->m_dwNpcID )
		pTEXCEPT = m_vMAP.GetTNPCINST(m_pTNODEPATH->m_dwNpcID);

	CExceptCol vExceptCol_m_pTNODEPATH(pTEXCEPT);
	pTEXCEPT = NULL;

	if( m_pTAUTOPATH && m_pTAUTOPATH->m_dwNpcID )
		pTEXCEPT = m_vMAP.GetTNPCINST(m_pTAUTOPATH->m_dwNpcID);

	CExceptCol vExceptCol_m_pTAUTOPATH(pTEXCEPT);
	pTEXCEPT = NULL;

	if( m_pTNODEPORTAL && m_pTNODEPORTAL->m_wNpcID )
		pTEXCEPT = m_vMAP.GetTNPCINST(m_pTNODEPORTAL->m_wNpcID);

	CExceptCol vExceptCol_m_pTNODEPORTAL(pTEXCEPT);
	pTEXCEPT = NULL;

	if( m_pTAUTOPORTAL && m_pTAUTOPORTAL->m_wNpcID )
		pTEXCEPT = m_vMAP.GetTNPCINST(m_pTAUTOPORTAL->m_wNpcID);

	CExceptCol vExceptCol_m_pTAUTOPORTAL(pTEXCEPT);
	pTEXCEPT = NULL;

	if( pTNODEPATH && pTPANNEL && pTPANNEL->m_bShowPath )
	{
		bBuild = m_pTNODEPATH != pTNODEPATH ? TRUE : FALSE;

		if(!bBuild)
		{
			D3DXVECTOR2 vTDEST = GetTNODEGOAL( pTNODEPATH, m_pTNODEPORTAL);
			D3DXVECTOR3 vTDIR = m_vTNODEPATH[0] - vTPOS;
			vTDIR.y = 0.0f;
			FLOAT fMOVE = D3DXVec3Length(&vTDIR);

			if( fMOVE > TTOTAL_NODE_LENGTH + TGUIDE_NODE_LENGTH || !m_vMAP.CanMove( m_pMainChar, &vTPOS, &m_vTNODEPATH[0]) )
				bBuild = TRUE;
			else if( fabs(m_vTNODEPATH[0].x - vTDEST.x) > 0.01f || fabs(m_vTNODEPATH[0].z - vTDEST.y) > 0.01f )
			{
				BYTE bCanMove = TRUE;
				FLOAT fDIST = 0.0f;
				...
				{
					if( fMOVE > TTOTAL_NODE_LENGTH + TGUIDE_NODE_LENGTH )
					{
						bBuild = TRUE;
						break;
					}

					if( fabs(m_vTNODEPATH[i + 1].x - vTDEST.x) < 0.01f &&
						fabs(m_vTNODEPATH[i + 1].z - vTDEST.y) < 0.01f )
						break;
					fDIST += vDIST[i];

					if( fMOVE > fDIST && m_vTNODEPATH[i + 1] != m_vTNODEPATH[i] )
					{
						bBuild = TRUE;
						break;
					}

					vTDIR = m_vTNODEPATH[i + 1] - m_vTNODEPATH[i];
					vTDIR.y = 0.0f;

					FLOAT fLength = D3DXVec3Length(&vTDIR);
					fMOVE += fLength;

					if( fDIST + 0.01f > fMOVE )
					{
						bBuild = TRUE;
						break;
					}

					if( bCanMove && fLength > 0.0f )
					{
						D3DXPLANE vTPLANE;
						vTDIR /= fLength;
						...
						if( D3DXPlaneDotCoord( &vTPLANE, &m_vTNODEPATH[i + 1]) * D3DXPlaneDotCoord( &vTPLANE, &m_vTNODEPATH[i]) < 0.0f )
							if(m_vMAP.CanMove( m_pMainChar, &vTPOS, &m_vTNODEPATH[i + 1]))
							{
								bBuild = TRUE;
								break;
							}
		...
		if(bBuild)
		{
			D3DXVECTOR3 vTNODEPREV = m_vTNODEPATH[0];
			...
			if(!BuildTNODEPATH(
				...
				TNODEGUIDE_COUNT + 1,
				TPATH_CASH_NODE,
				m_pTNODEPATH != pTNODEPATH ? TRUE : FALSE))
			{
				ShowInfoChat( CTChart::LoadString(TSTR_ERROR_FIND_NODE_FAILED), TUISND_TYPE_ERROR);
			...
			{
				if( m_pTNODEPATH == pTNODEPATH &&
					fabs(m_vTNODEPATH[0].x - vTNODEPREV.x) < 0.01f &&
					fabs(m_vTNODEPATH[0].z - vTNODEPREV.z) < 0.01f )
					bBuild = FALSE;

				m_pTNODEPATH = pTNODEPATH;
			}
		}
	}
	else
	...
	if( m_pTAUTOPATH == &m_vTAUTOTARGET && (!pTPANNEL || !pTPANNEL->m_bAutoPath) )
	{
		if( m_bTAUTOPATH || D3DXPlaneDotCoord( &m_vTAUTOPLANE, &vTPOS) > 0.0f )
		{
			D3DXVECTOR3 vTAUTOPREV = m_vTAUTOPATH;
			...
			else if( !m_bTAUTOPATH && ((
				fabs(m_vTAUTOPATH.x - m_vTAUTOPREV.x) < 0.01f &&
				fabs(m_vTAUTOPATH.z - m_vTAUTOPREV.z) < 0.01f) || (
				fabs(m_vTAUTOPATH.x - vTAUTOPREV.x) < 0.01f &&
				fabs(m_vTAUTOPATH.z - vTAUTOPREV.z) < 0.01f)) )
			{
				m_vTAUTOPATH = vTAUTOPREV;
				m_bTAUTOREV = TRUE;
			}
			else
			{
	...
	else if( !pTNODEPATH || !pTPANNEL || !pTPANNEL->m_bAutoPath )
	{
		ZeroMemory( &m_vTAUTOPLANE, sizeof(D3DXPLANE));
	...
	else if( bBuild || m_pTAUTOPATH != pTNODEPATH || D3DXPlaneDotCoord( &m_vTAUTOPLANE, &vTPOS) > 0.0f )
	{
		D3DXVECTOR3 vTAUTOPREV = m_vTAUTOPATH;
		BYTE bFind = TRUE;

		if(!pTPANNEL->m_bShowPath)
		{
			bFind = BuildTNODEPATH(
				m_pCtrlHost,
				pTNODEPATH,
				&m_pTAUTOPORTAL,
				&m_vTAUTOPATH,
				vDIST, 1,
				TPATH_CASH_AUTO,
				bBuild || m_pTAUTOPATH != pTNODEPATH ? TRUE : FALSE);
		}
		else
		{
			m_pTAUTOPORTAL = m_pTNODEPORTAL;
			m_vTAUTOPATH = m_vTNODEPATH[0];
		}
		...
		else if( m_pTAUTOPATH == pTNODEPATH && ((
			fabs(m_vTAUTOPATH.x - m_vTAUTOPREV.x) < 0.01f &&
			fabs(m_vTAUTOPATH.z - m_vTAUTOPREV.z) < 0.01f) || (
			fabs(m_vTAUTOPATH.x - vTAUTOPREV.x) < 0.01f &&
			fabs(m_vTAUTOPATH.z - vTAUTOPREV.z) < 0.01f)) )
	...
	if(!m_pTNODEPATH)
		for( int i=0; i<TNODEGUIDE_COUNT; i++)
			m_vTNODEGUIDE[i].RemoveOBJSFX();

	return bResult;
}

void CTClientGame::CalcTNODEGUIDE( DWORD dwTick)
{
	BYTE bUPDATE = ResetTPATHDATA();

	if( m_pTAUTOPATH && m_bAutoRun )
		m_bAutoRun = FALSE;

	if( m_pTAUTOPATH && CanLookAt() && m_pMainChar->CanAutoPath() )
	{
		D3DXVECTOR2 vTDEST = GetTNODEGOAL( m_pTAUTOPATH, m_pTAUTOPORTAL);
		BYTE bTGOAL = fabs(vTDEST.x - m_vTAUTOPATH.x) < 0.01f && fabs(vTDEST.y - m_vTAUTOPATH.z) < 0.01f ? TRUE : FALSE;

		if( bUPDATE && !m_bTAUTOPAUSE )
		{
			WORD wNpcID = m_pTAUTOPORTAL ? m_pTAUTOPORTAL->m_wNpcID : WORD(m_pTAUTOPATH->m_dwNpcID);
			CTClientObjBase *pTARGET = wNpcID && bTGOAL ? m_vMAP.GetTNPCINST(wNpcID) : NULL;

			if(!pTARGET)
			{
				m_bMoveType = TMAINMOVE_GROUND;
				m_dwMoveGM = 0;
				m_fBOUND = bTGOAL ? TPATH_DIST / 2.0f : -0.01f;

				m_vTGROUND = bTGOAL ? m_vTAUTOPATH : m_vTAUTOPATH + TPATH_DIST * GetTAUTOPATHDIR();
			}
			else
			{
				...
				m_dwMoveGM = m_pTAUTOPORTAL ? GM_NPC_TRADE : GM_NPC_TALK;
				m_fBOUND = POPUP_DIST;
			}
		...
		else if( m_bTAUTOREV && !bTGOAL )
			m_vTGROUND += GetTHOSTDIR();

		if( !m_bTAUTOPAUSE && m_bMoveType == TMAINMOVE_GROUND )
			m_vTGROUND.y = m_pCtrlHost->GetPositionY();

		if( m_bTAUTOJUMP && !m_pCtrlHost->IsJump() )
		{
			SendCS_JUMP_REQ(
				m_pCtrlHost->m_dwID,
				m_pCtrlHost->m_bType,
	...
	if(m_pTNODEPATH)
	{
		D3DXVECTOR2 vTDEST = GetTNODEGOAL( m_pTNODEPATH, m_pTNODEPORTAL);
		D3DXVECTOR3 vTPOS = m_pMainChar->GetPosition();
		D3DXVECTOR3 vTDIR = m_vTNODEPATH[0] - vTPOS;

		vTPOS.y += m_pMainChar->m_fSizeY / 2.0f;
		vTDIR.y = 0.0f;
		...
		if( fLength < vDIST[0] )
		{
			vTPOS.x = m_vTNODEPATH[0].x;
			vTPOS.z = m_vTNODEPATH[0].z;
			fMOVE = fLength;
		}

		while( nIndex < TNODEGUIDE_COUNT )
		{
			vTDIR = m_vTNODEPATH[nIndex + 1] - vTPOS;
			vTDIR.y = 0.0f;
			fLength = D3DXVec3Length(&vTDIR);
			fDIST = vDIST[nIndex] - fMOVE;

			if( fDIST + 0.01f > fLength )
			{
				while( nIndex < TNODEGUIDE_COUNT )
				{
					m_vTNODEGUIDE[nIndex].m_bVisible = FALSE;
					m_vTNODEGUIDE[nIndex].RemoveOBJSFX();
			...
			vTPOS.y = m_vMAP.GetHeight(
				m_pMainChar,
				NULL,
				&vTPOS,
				0.0f,
				FALSE) + m_pMainChar->m_fSizeY / 2.0f;

			D3DXMatrixRotationY(
				&m_vTNODEGUIDE[nIndex].m_vWorld,
				D3DX_PI + atan2f( vTDIR.x, vTDIR.z));

			m_vTNODEGUIDE[nIndex].m_vWorld._41 = vTPOS.x;
			m_vTNODEGUIDE[nIndex].m_vWorld._42 = vTPOS.y;
			m_vTNODEGUIDE[nIndex].m_vWorld._43 = vTPOS.z;
			m_vTNODEGUIDE[nIndex].m_bVisible = TRUE;

			m_vTNODEGUIDE[nIndex].CalcTick(
				m_pDevice->m_pDevice,
				dwTick);
			...
			if( fLength < vDIST[nIndex] )
			{
				vTPOS.x = m_vTNODEPATH[nIndex].x;
				vTPOS.z = m_vTNODEPATH[nIndex].z;
				fMOVE = fLength;
			}
		}
	}
...
D3DXVECTOR3 CTClientGame::GetTAUTOPATHDIR()
{
	D3DXVECTOR3 vTDIR = m_vTAUTOPATH - m_pCtrlHost->GetPosition();
	vTDIR.y = 0.0f;
	FLOAT fLength = D3DXVec3Length(&vTDIR);
...
D3DXVECTOR3 CTClientGame::GetTHOSTDIR()
{
	D3DXVECTOR3 vTDIR( 0.0f, 0.0f, -1.0f);
	D3DXMATRIX vTROT;

	D3DXMatrixRotationY(
		&vTROT,
		FLOAT(m_pCtrlHost->m_wDIR) * D3DX_PI / 900.0f);
...
void CTClientGame::DoTAUTOPATH( LPTTERMPOS pTAUTOPATH)
{
	ReleaseMoveType();
	memcpy( &m_vTAUTOTARGET, pTAUTOPATH, sizeof(TTERMPOS));
	m_pTAUTOPATH = &m_vTAUTOTARGET;
	m_bTAUTOPATH = TRUE;
}
		...
		if(!pTARGET || bOnSDT_RUNAWAY)
		{
			...
			if( bActionID != TA_STAND || fDIST > TRECALL_STAND_BOUND * TRECALL_STAND_BOUND || fDIST < fRadius * fRadius )
			{
				if( !bOnSDT_RUNAWAY )
				{
					...
					vTARGET = pTRECALL->AdjustRoamTarget(
						FindPC(pTRECALL->m_dwHostID),
						&m_vMAP,
						pTDIR,
						vTARGET );
...
int CTClientGame::OnCommand( TCOMMAND Command)
{
...
#ifdef MAGICITEMSHOP
	ON_GM_COMMAND(GM_MAGICITEM_BUY)
#endif
	ON_GM_COMMAND(GM_MONSTER_BUY)
	ON_GM_COMMAND(GM_PORTAL_REQ)
	ON_GM_COMMAND(GM_PORTAL_REQ_DIRECT)
...
#ifdef MAGICITEMSHOP
	ON_GM_COMMAND(GM_MAGICITEM_DO_BUY)
#endif
	...
	ON_GM_COMMAND(GM_QUEST_MAPOPEN)
	ON_GM_COMMAND(GM_QUEST_MAPCLOSE)
	...
	ON_GM_COMMAND(GM_GUILD_DONATE_PVPPOINT)
	...
	ON_GM_COMMAND( GM_GMP_TACTICS_SELF_KICK )
...
//	ON_GM_COMMAND( GM_TEXTOOL_COMPO )
	...
	ON_GM_COMMAND(GM_EXIT_TUTORIAL)
	...
	ON_GM_COMMAND(GM_SELECT_DO_AID)
	ON_GM_COMMAND(GM_SELECT_AID_D)
	ON_GM_COMMAND(GM_SELECT_AID_C)
	ON_GM_COMMAND(GM_NPC_MEETING)
	ON_GM_COMMAND(GM_NPC_MERCENARY)
	ON_GM_COMMAND(GM_NPC_NATIONALITY1)
	ON_GM_COMMAND(GM_NPC_NATIONALITY2)
	ON_GM_COMMAND(GM_DO_CHANGE_TO_BROIR)
	ON_GM_COMMAND(GM_SELECT_MY_COUNTRY)
	ON_GM_COMMAND(GM_SELECT_MY_COUNTRY_D)
	ON_GM_COMMAND(GM_SELECT_MY_COUNTRY_C)
	ON_GM_COMMAND(GM_DO_SELECT_COUNTRY)
	ON_GM_COMMAND(GM_ANSWER_MEETING_IGNORE)
	ON_GM_COMMAND(GM_ANSWER_METTING_OK)
	ON_GM_COMMAND(GM_ANSWER_MEETING_NO)
	ON_GM_COMMAND(GM_INVITE_MEETING_ROOM)
	ON_GM_COMMAND(GM_CHALLENGE_JOIN)
	ON_GM_COMMAND(GM_CHALLENGE_GIVEUP)
	ON_GM_COMMAND(GM_CHALLENGE_EVENT)
	ON_GM_COMMAND(GM_NPC_DUEL3)
	ON_GM_COMMAND(GM_NPC_DUEL7)
	ON_GM_COMMAND(GM_REALLY_SELECT_COUNTRY)
	ON_GM_COMMAND(GM_CHALLENGE_JOIN_DO)
	ON_GM_COMMAND(GM_CHALLENGE_GIVEUP_DO)
	ON_GM_COMMAND(GM_SELECT_AID_GIVEUP)
	ON_GM_COMMAND(GM_SELECT_AID_GIVEUP_DO)
	ON_GM_COMMAND(GM_CHANGE_ORG_COUNTRY)

	ON_GM_COMMAND(GM_GUILD_TACTICS_SELF_QUIT)
#ifdef ADD_SKYGARDEN
	ON_GM_COMMAND( GM_ASSISTANT_REQ )
	ON_GM_COMMAND( GM_ASSISTANT_OK )
	ON_GM_COMMAND( GM_ASSISTANT_NO )
	ON_GM_COMMAND( GM_ASSISTANT_BUSY )
	ON_GM_COMMAND( GM_ASSISTANT_KICK )
	ON_GM_COMMAND( GM_ASSISTANT_SELFKICK )
#endif
	ON_GM_COMMAND(GM_NODE_TELEPORT_YES)
	ON_GM_COMMAND(GM_NODE_TELEPORT_NO)
	ON_GM_COMMAND(GM_TUTORIAL_START)
	ON_GM_COMMAND(GM_TUTORIAL_SKIP)
	ON_GM_COMMAND(GM_SELECT_COUNTRY_DLG)
...
int CTClientGame::OnSessionMsg( CTachyonSession *pSession, CPacket *pPacket)
{
...
#ifdef MAGICITEMSHOP
	ON_TRECEIVE(CS_MAGICITEMBUY_ACK)
#endif
...
#ifdef ADD_SKYGARDEN
	ON_TRECEIVE(CS_ENTERSKYGARDEN_ACK)
#endif
	...
	ON_TRECEIVE( CS_WARCOUNTRYBALANCE_ACK )
	ON_TRECEIVE( CS_MEETINGROOM_ACK )

	ON_TRECEIVE( CS_ARENA_ACK )
	ON_TRECEIVE( CS_ARENATEAM_ACK )

#ifdef ADD_SKYGARDEN
	ON_TRECEIVE( CS_ASSISTANT_ACK )
	ON_TRECEIVE( CS_ASSISTANTRESULT_ACK )
	ON_TRECEIVE( CS_ASSISTANTADD_ACK )
	ON_TRECEIVE( CS_ASSISTANTDEL_ACK )
	ON_TRECEIVE( CS_ASSISTANTATTR_ACK )
	ON_TRECEIVE( CS_ASSISTANTSTAT_ACK )
#endif

#ifdef ADD_CMGIFT
	ON_TRECEIVE( CS_CMGIFT_ACK )
#endif
...
void CTClientGame::ReleaseForTELEPORT()
{
	...
	if(m_bTAUTOPAUSE)
	{
		...
		if( m_pTAUTOPATH != &m_vTAUTOTARGET )
			m_pTAUTOPATH = NULL;
		else
			m_bTAUTOPATH = TRUE;
	}
...
#ifdef MAGICITEMSHOP
	DisableUI(TFRAME_MAGICITEM_SHOP);
#endif
...
#ifdef MAGICITEMSHOP
		m_vTFRAME[TFRAME_MAGICITEM_SHOP] == pUI ||
#endif
...
void CTClientGame::OnActivateKEY( WORD wSCAN)
{
	...
	case TKEY_JUMP				:
		if( !m_pMainChar->IsDead() && !m_pMainChar->IsBlock() && !m_pMainChar->IsDown() && !m_pMainChar->m_bPrivateShop )
		{
			...
			if( !m_pMainChar->m_bGhost && !m_pCtrlHost->IsJump() )
			{
				SendCS_JUMP_REQ(
					m_pCtrlHost->m_dwID,
					m_pCtrlHost->m_bType,
...
#ifdef ADD_SNOW
		if( m_pMainChar->m_pTREGION->m_bWeather == TWEATHER_SNOW )
			StartSnowEvent();
		else
			EndSnowEvent();
#endif
...
void CTClientGame::FindSlaveRecallByHost( DWORD dwHostID, LPVTRECALL vTRECALL )
{
	MAPRECALL::iterator it, end;
	it = m_mapSLAVERECALL.begin();
	end = m_mapSLAVERECALL.end();
	for(; it != end ; ++it )
	{
		CTClientRecall* pTRECALL = it->second;
		if( pTRECALL )
		{
			if( pTRECALL->m_dwHostID == dwHostID )
				vTRECALL->push_back( pTRECALL );
		}
	}
}
...
void CTClientGame::FindRecallByHost( DWORD dwHostID, LPVTRECALL vTRECALL )
{
	MAPRECALL::iterator it, end;
	it = m_mapRECALL.begin();
	end = m_mapRECALL.end();
	for(; it != end ; ++it )
	{
		CTClientRecall* pTRECALL = it->second;
...
void CTClientGame::OnLButtonDown( UINT nFlags, CPoint pt)
{
	...
	else
	{
		...
		if(HitTest(pt))
		{
			...
			else if( m_pTMinimap->IsVisible() && !m_pTMinimap->m_bMinimize )
				CheckTMINIMAP( nFlags, pt);
...
#ifdef MAGICITEMSHOP
							||
							m_vTFRAME[TFRAME_MAGICITEM_SHOP]->IsVisible()
#endif
							)
						{
...
#ifdef MAGICITEMSHOP
		case TFRAME_MAGICITEM_SHOP	:
			{
				CTMagicItemShopDlg* pMagicShopDlg = static_cast<CTMagicItemShopDlg*>(m_vTFRAME[TFRAME_MAGICITEM_SHOP]);
				if( pMagicShopDlg->GetList()->GetHitItem(pt) )
					OnGM_MAGICITEM_BUY();
			}
			break;
#endif
		...
		if(bDEFEND)
		{
			...
			if( bBroadCast )
			{
				SendCS_DEFEND_REQ(
					...
					pTSKILLDATA->m_bAidAtkCountryID,
			...
			else
			{
				if( pTDEFEND )
				{
					...
					{
						m_pSESSION->SendCS_ACDCLOSE_REQ( 1 );
						return FALSE;
					}
				}
...
void CTClientGame::DisableUI( BYTE bUI, BOOL bShow)
{
	if( m_vTFRAME[bUI] && m_vTFRAME[bUI]->IsVisible() )
	{
		...
		case TFRAME_MAGICITEM_SHOP:
			{
				CTMagicItemShopDlg* pMagicShopDlg = (CTMagicItemShopDlg*) m_vTFRAME[bUI];
			}
			break;
#endif
		case TFRAME_PORTAL_SHOP:
...
void CTClientGame::EnableUI( BYTE bUI)
{
	if(m_vTFRAME[bUI] && !m_vTFRAME[bUI]->IsVisible())
	{
		...
		case TFRAME_AUCTION:
			{
				CTFrameGroupBase* pBase = (CTFrameGroupBase*) m_vTFRAME[bUI];
				pBase->m_nSelectedFrame = TAUCTION_SEARCH;
			}
			break;
		}
...
void CTClientGame::ResetPlayerINFO( BYTE bContinue)
{
	...
	for( int i=0 ; i < 3 ; ++i )
	{
		if( i < m_vTASSISTANT.size() )
		{
			if( m_vTASSISTANT[ i ]->m_dwCharID == m_vTASSISTANT[ i ]->m_dwCommanderID )
				m_pTASSISTANT[ i ]->SetCurImage( m_vTASSISTANT[ i ]->m_bDie ? 2 : 1 );
			else
				m_pTASSISTANT[ i ]->SetCurImage( m_vTASSISTANT[ i ]->m_bDie ? 4 : 3 );
		}
		else
			m_pTASSISTANT[ i ]->SetCurImage( 0 );
	}
#endif
}
...
void CTClientGame::DropItem( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint point )
{
...
#ifdef MAGICITEMSHOP
		m_vDRAG.m_bFrameID == TFRAME_MAGICITEM_SHOP ||
#endif
...
#ifdef MAGICITEMSHOP
			case TFRAME_MAGICITEM_SHOP	: DropItem_InvenToItemshop(nFlags, bTargetUI, bSlotID); break;
#endif
	...
	case TFRAME_GAUGE		:			
		{
			...
			case TFRAME_PET_MANAGE	:
			default					:	DropItem_MainUIToDefault(nFlags, bTargetUI, bSlotID, point); break;
			}
...
void CTClientGame::DropItem_MainUIToDefault( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint ptDrag )
{
	if(!m_pSESSION)
		return;
	...
	case TMAINSLOTID_HOTKEY_DEF		:
		{
			if( pTGaugePannel->m_pTHOTKEYFRM[TMAINSLOTID_HOTKEY_DEF]->HitTest(ptDrag) ||
				pTGaugePannel->m_pTHOTKEYFRM[TMAINSLOTID_HOTKEY_SUB1]->HitTest(ptDrag) )
				return ;
...
void CTClientGame::DropItem_ShopToInven( UINT nFlags, BYTE bTargetUI, BYTE bSlotID )
{
	...
	if( m_vDRAG.m_bFrameID == TFRAME_ITEM_SHOP )
		OnGM_ITEM_BUY();
#ifdef MAGICITEMSHOP
	else if( m_vDRAG.m_bFrameID == TFRAME_MAGICITEM_SHOP )
		OnGM_MAGICITEM_BUY();
#endif
	else
		OnGM_BUY_CASHITEM();
...
void CTClientGame::BuildNPCMENU( CTClientNpc *pTNPC)
{
	...
	if(!pTNPC->m_pTPOPUP || (pTNPC->m_bCamp != 0 && pTNPC->m_bCamp != m_pMainChar->m_bCamp) )
		return;
	...
	else if( pTNPC->m_bContryID == m_pMainChar->GetWarCountry() &&
		(pTNPC->m_bNPCType != TNPC_AUCTION) )
	{
		bAlliance = TRUE;
	}
	else
	{
		...
		else
		{
			...
			if( m_pMainChar->m_bContryID == TCONTRY_B )
			{
				if( pTNPC->m_bNPCType == TNPC_PORTAL ||
					pTNPC->m_bNPCType == TNPC_AUCTION ||
					pTNPC->m_bNPCType == TNPC_ITEM ||
					pTNPC->m_bNPCType == TNPC_GUILD )
					bAlliance = TRUE;
			}
		}
	}
	...
	for( BYTE i=0; i<BYTE(pTNPC->m_pTPOPUP->m_vTPOPUP.size()); i++)
		if(pTNPC->m_pTPOPUP->m_vTPOPUP[i]->m_bGhost & bGhost)
		{
			...
			case GM_NPC_TRADE:
				{
					...
					if( (bAlliance || pTNPC->m_bContryID == TCONTRY_B || m_pMainChar->m_bContryID == TCONTRY_B) &&
						pTNPC->m_bNPCType == TNPC_AUCTION )
						bADD = TRUE;
				}
				break;
			...
			case GM_NPC_GUILD_DEL_CANCEL	:
				if( !m_pMainChar->m_dwGuildID || !bAlliance || m_pMainChar->m_bGuildDuty != GUILD_DUTY_CHIEF || m_pMainChar->m_dwTacticsID != 0 )
					bADD = FALSE;
...
void CTClientGame::BuildPCMENU( CTClientChar *pTCHAR)
{
	static DWORD dwMenuID[][2] = {
		...
		{ GM_FRIEND_REQ, TSTR_MENU_FRIEND_REQ},
#ifdef ADD_SKYGARDEN
		{ GM_ASSISTANT_REQ, TSTR_MENU_ASSISTANT_INVITE },
		{ GM_ASSISTANT_KICK, TSTR_MENU_ASSISTANT_KICK },
#endif
		{ GM_GUILD_REQ, TSTR_MENU_GUILD_REQ},
		{ GM_SHOW_INVITE_TACTICS, TSTR_MENU_TACTICS_REQ},
	...
	BOOL bBattleAlliacne = m_pMainChar->GetWarCountry() == pTCHAR->GetWarCountry(); //m_pMainChar->IsAlliance(pTCHAR);
	BOOL bTradeAlliance = bBattleAlliacne || pTCHAR->m_bContryID == TCONTRY_B || m_pMainChar->m_bContryID == TCONTRY_B;
	BOOL bGuildFriendAlliance = m_pMainChar->m_bContryID == pTCHAR->m_bContryID;
	BOOL bTargetChallengeGame = pTCHAR->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE;
	...
	for( BYTE i=0; i< dwCount; i++)
	{
		...
		case GM_TRADE_REQ			:
			if( IsMiniGameBattle() ||
				...
				bTargetChallengeGame ||
				!bTradeAlliance ||
				pTCHAR->m_bPrivateShop )
				bADD = FALSE;
		...
		case GM_PARTY_REQ			:
			if( m_bTournamentWatching ||
				!bBattleAlliacne ||
				(wSquadID && (m_pMainChar->m_dwChiefID != m_pMainChar->m_dwID || wSquadID == wPartyID)) )
				bADD = FALSE;
		...
		case GM_FRIEND_REQ			:
			{
				if( IsMiniGameBattle() || m_bTournamentWatching || bTargetChallengeGame || !bGuildFriendAlliance )
					bADD = FALSE;
				else
				{
					CTMsgGroupList* pList = m_pMessengerBase->GetMsgGroupList();
			...
			break;
#ifdef ADD_SKYGARDEN
		case GM_ASSISTANT_REQ:
			{
				if( m_bTournamentWatching ||
					!bBattleAlliacne ||
					pTCHAR->m_wAssistantID != 0 )
					bADD = FALSE;
			}
			break;

		case GM_ASSISTANT_KICK:
			{
				if( m_bTournamentWatching ||
					!bBattleAlliacne ||
					m_pMainChar->m_bLevel < 80 ||
					pTCHAR->m_wAssistantID == 0 ||
					pTCHAR->m_strAssistantCommander != m_pMainChar->m_strNAME )
					bADD = FALSE;
			}
			break;
#endif
		case GM_GUILD_REQ			:
			if( IsMiniGameBattle() ||
				m_bTournamentWatching ||
				bTargetChallengeGame ||
				!bGuildFriendAlliance ||
				!m_pMainChar->m_dwGuildID || 
				pTCHAR->m_dwGuildID ||
				m_pMainChar->m_bGuildDuty == GUILD_DUTY_NONE )
			{
				bADD = FALSE;
			}
		...
		case GM_SHOW_INVITE_TACTICS:
			if( IsMiniGameBattle() || m_bTournamentWatching || bTargetChallengeGame || !bBattleAlliacne || !m_pMainChar->m_dwGuildID || 
		...
		case GM_CORPS_REQ			:
			if( m_bTournamentWatching || !bBattleAlliacne || !wSquadID ||
				m_pMainChar->m_dwChiefID != m_pMainChar->m_dwID || (
				m_pTRSCS->m_vTCORPS.m_wID && wSquadID != m_pMainChar->m_wCommanderID) )
				bADD = FALSE;
		...
		case GM_BLOCK_REQ :
			if ( IsMiniGameBattle() || m_bTournamentWatching || !bBattleAlliacne  )
				bADD = FALSE;
			else
			{
...
void CTClientGame::BuildMAINCHARMENU()
{
	static DWORD dwMenuID[][2] = {
		...
		{ GM_ASSISTANT_SELFKICK, TSTR_MENU_ASSISTANT_SELFKICK },
	...
	for( BYTE i=0; i<TMAINCHARMENU_COUNT; i++)
	{
		...
		switch(dwMenuID[i][0])
		{
		case GM_ASSISTANT_SELFKICK:
			{
				if( 0 == m_pMainChar->m_wAssistantID )
					bADD = FALSE;
			}
			break;
		...
		case GM_OPEN_PRIVSHOP_FOR_SELL:
			if(IsMiniGameBattle() || m_bTournamentWatching || 
...
BOOL CTClientGame::CanDuel(CTClientChar* pTARGET)
{
	...
	if( m_pMainChar->m_pTREGION->m_bInfo == TREGIONINFO_BATTLE )
		return FALSE;

	if( !m_pMainChar->IsAlliance(pTARGET) && m_pMainChar->m_pTREGION->m_bInfo != TREGIONINFO_B )
		return FALSE;
	...
	case IK_COUNTRY:
		{
			...
			m_pMainWnd->MessageBoxYesNo(
				TSTR_NATIONALITY_RECHANGE,
				TSTR_YES,
				TSTR_NO,
				TCOMMAND( GM_CHANGE_ORG_COUNTRY, dwPARAM ),
				GM_CLOSE_MSGBOX,
				GM_CLOSE_MSGBOX,
				FALSE );
		}
	...
	case SKILL_SUCCESS		:
		{
			...
			{
				if(pTSKILL->m_pTSKILL->m_wItemID[m_pMainChar->GetSkillItemType(pTSKILL->m_pTSKILL)])
					m_pMainChar->m_bShoot = TRUE;
				else
					m_pMainChar->m_bShoot = FALSE;
...
DWORD CTClientGame::GetObjectColor( CTClientObjBase *pTOBJ)
{
	...
	if( m_bChallengeGame )
	{
		...
		if( pTOBJHOST &&
			(pTOBJHOST->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE) &&
			(pTOBJHOST->m_bChallengeTeam != m_pMainChar->m_bChallengeTeam) )
			return TCOLOR_ENEMY_HEAVY;
	...
	else
	{
		...
		if( pTOBJHOST )
		{
			if( pTOBJHOST->m_bChallengeTeam == TCHALLENGE_TEAM_RED )
				return TCOLOR_TOURNAMENT_RED;

			if( pTOBJHOST->m_bChallengeTeam == TCHALLENGE_TEAM_BLUE )
...
BYTE CTClientGame::IsInBattle(CTClientObjBase *pTATTACK, CTClientObjBase *pTDEFEND)
{
	...
	if( m_bChallengeGame &&
		m_bChallengeStatus == BS_BATTLE &&
		(pTATTACK == m_pMainChar || pTDEFEND == m_pMainChar) &&
		pTATTACK->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE &&
		pTDEFEND->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE &&
		pTATTACK->m_bChallengeTeam != pTDEFEND->m_bChallengeTeam )
		return TRUE;
									 ...
									 LPTSKILL pTSKILL)
{
	...
	case OT_MON:
		{
			if( pTATTACK && pTDEFEND &&
				(pTATTACK->m_bContryID == TCONTRY_B && pTATTACK->m_bAidCountryID == TCONTRY_N) &&
				(pTDEFEND->m_bContryID == TCONTRY_C || pTDEFEND->m_bContryID == TCONTRY_D ) )
				return SKILL_WRONGTARGET;
		}
		break;
	}
							  ...
							  LPTSKILL pTSKILL)
{
	...
	if( pTDEFEND == m_pMainChar &&
		m_pMainChar->m_bCanDefend == FALSE)
	{
		m_pSESSION->SendCS_ACDCLOSE_REQ( 4 );
	}
...
{
	if( m_bDrawGROUND || m_bGroundLOCK || (m_bMoveType == TMAINMOVE_GROUND && !m_pTAUTOPATH) )
		return TRUE;
...
void CTClientGame::CheckTMINIMAP( UINT nFlags, CPoint point)
{
	if(!m_vMAP.m_pMAP)
		return;

	CRect rect(m_pTMinimap->m_rcAREA);
	CPoint vPOS;

	m_pTMinimap->GetComponentPos(&vPOS);
	rect.OffsetRect(
		vPOS.x,
		vPOS.y);
	m_pTMinimap->ComponentToScreen(&rect);

	if( rect.PtInRect(point) && !m_pTMinimap->CTClientUIBase::HitTest(point) )
	{
		FLOAT fSCALE = m_vMAP.m_fTSCALE / FLOAT(m_vMAP.m_pMAP->m_nUnitLength * m_vMAP.m_pMAP->m_nTileLength);
		TTERMPOS vTAUTOPATH;
		...
		vTAUTOPATH.m_dwMapID = m_vMAP.m_wMapID;
...
void CTClientGame::CheckTRSCS( UINT nFlags, CPoint point)
{
	...
	rect.OffsetRect(
		vPOS.x,
		vPOS.y);
	m_pTRSCS->ComponentToScreen(&rect);

	if( rect.PtInRect(point) && !m_pTRSCS->CTClientUIBase::HitTest(point) )
	{
	...
	case OT_NPC:
		{
			...
			::SetCursor( m_pMainWnd->m_vTCURSOR[TCURSOR_HAND] );
		}
...
void CTClientGame::ReleaseMoveType()
{
	CTGaugePannel *pTPANNEL = (CTGaugePannel *) m_vTFRAME[TFRAME_GAUGE];
	...
	if( pTPANNEL && pTPANNEL->m_bAutoPath )
	{
		pTPANNEL->m_bAutoPath = FALSE;
...
void CTClientGame::ResetKICKOUTMAP()
{
	...
	if( m_vTFRAME[TFRAME_GAUGE] )
	{
		static_cast<CTGaugePannel*>( m_vTFRAME[TFRAME_GAUGE] )->DettachSiegeAlarm();
		static_cast<CTGaugePannel*>( m_vTFRAME[TFRAME_GAUGE] )->DettachExitTutorial();
	}
}
...
BOOL CTClientGame::IsShowMinimap()
{
	...
	return !m_pTRSCS->IsVisible() &&
		!m_bTournamentBattle &&
		!m_bTournamentWatching &&
		!(m_bChallengeGame || m_pMainChar->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE );
}
...
void CTClientGame::ExitTutorial()
{
	if(m_pSESSION)
		...
		case TCONTRY_B	: m_pSESSION->SendCS_TELEPORT_REQ(TSTART_B_PORTAL_ID); break;
		case TCONTRY_PEACE : m_pSESSION->SendCS_TELEPORT_REQ(TSTART_P_PORTAL_ID); break;
		}
}
...
void CTClientGame::SetChannel( BYTE bNewChannel )
{
	m_pMainWnd->m_bChannel = bNewChannel;

	for( int i=0 ; i < INT(m_pMainWnd->m_pTGROUP->m_vChannel.size()) ; ++i )
		if( m_pMainWnd->m_bChannel == m_pMainWnd->m_pTGROUP->m_vChannel[ i ]->bID )
		{
			m_pMainWnd->m_pTGROUP->m_bSelectedChannelIndex = (BYTE) i;
			break;
		}
...
void CTClientGame::ResetSkillSLAVERECALL()
{
	MAPRECALL::iterator itSR, endSR;
	itSR = m_mapSLAVERECALL.begin();
	endSR = m_mapSLAVERECALL.end();
	for(; itSR != endSR ; ++itSR )
	{
		CTClientRecall* pSR = itSR->second;
		pSR->ResetDefaultSkill();
	}
}

void CTClientGame::DoEventFirstConnect()
{
	if(m_bFirstConnect)
	{
		switch( m_pMainChar->m_bContryID )
...
void CTClientGame::ResetDisplayChallengeInfo()
{
	DWORD dwBlueChief = 0;
	DWORD dwRedChief = 0;
	DWORD dwBlueCount = 0;
	...
	for(; it != end ; ++it )
	{
		CTClientChar* pCHAR = it->second;
		if( NULL == pCHAR )
			continue;

		if( m_pMainChar->m_wChallengeArenaID == pCHAR->m_wChallengeArenaID )
		{
			if( pCHAR->m_bChallengeTeam == TCHALLENGE_TEAM_RED )
			{
				if( !pCHAR->IsDead() && !pCHAR->m_bGhost )
					dwRedCount += 1;
				dwRedChief = pCHAR->m_dwChiefID ? pCHAR->m_dwChiefID : dwRedChief;
			}
			else if( pCHAR->m_bChallengeTeam == TCHALLENGE_TEAM_BLUE )
			{
				if( !pCHAR->IsDead() && !pCHAR->m_bGhost )
					dwBlueCount += 1;
				dwBlueChief = pCHAR->m_dwChiefID ? pCHAR->m_dwChiefID : dwBlueChief;
			}
		}
	}

	if( m_pMainChar->m_bChallengeTeam == TCHALLENGE_TEAM_RED )
	{
		if( !m_pMainChar->IsDead() && !m_pMainChar->m_bGhost )
			dwRedCount += 1;
		dwRedChief = m_pMainChar->m_dwChiefID ? m_pMainChar->m_dwChiefID : dwRedChief;
	}
	else if( m_pMainChar->m_bChallengeTeam == TCHALLENGE_TEAM_BLUE )
	{
		if( !m_pMainChar->IsDead() && !m_pMainChar->m_bGhost )
			dwBlueCount += 1;
		dwBlueChief = m_pMainChar->m_dwChiefID ? m_pMainChar->m_dwChiefID : dwBlueChief;
	}

	CTClientChar* pTBLUECHIEF = FindPC( dwBlueChief );
	CTClientChar* pTREDCHIEF = FindPC( dwRedChief );
	CString strBlueChief = pTBLUECHIEF ? pTBLUECHIEF->m_strNAME : _T("");
	CString strRedChief = pTREDCHIEF ? pTREDCHIEF->m_strNAME : _T("");

	CTGaugePannel* pDLG = static_cast<CTGaugePannel*>( m_vTFRAME[ TFRAME_GAUGE ] );
	pDLG->m_pTournamentInfo->SetChallengeInfo(
		strBlueChief,
		strRedChief,
		dwBlueCount,
		dwRedCount );
	pDLG->m_pTournamentInfo->ShowComponent(TRUE);
}

void CTClientGame::ClearTASSITANT()
{
	VTTASSISTANT::iterator it, end;
	it = m_vTASSISTANT.begin();
	end = m_vTASSISTANT.end();
	for(; it != end ; ++it )
	{
		delete (*it);
	}
...
bool sort_TASSITANT( const LPTASSISTANT& a, const LPTASSISTANT& b )
{
	if( a->m_wAssID == a->m_dwCommanderID )
		return true;
	
	if( b->m_wAssID == b->m_dwCommanderID )
		return false;

	return a->m_bLevel > b->m_bLevel;
}

void CTClientGame::SortTASSITANT()
{
	std::sort(
		m_vTASSISTANT.begin(),
		m_vTASSISTANT.end(),

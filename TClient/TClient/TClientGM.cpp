...
int CTClientGame::OnGM_LOBBY_QUIT()
{
	m_pMainWnd->MessageBoxYesNo(
		TSTR_QUIT_MSG,
		TSTR_YES,
...
int CTClientGame::OnGM_TRADE_REQ()
{
	...
	if(!m_pMainChar->IsTradeAlliance(m_pTARGET))
	{
...
int CTClientGame::OnGM_NPC_TRADE()
{
	...
	if( ((CTClientNpc*)m_pTARGET)->m_bCamp != 0 &&
		((CTClientNpc*)m_pTARGET)->m_bCamp != m_pMainChar->m_bCamp )
		return TERR_NONE;
	...
	if( m_pTARGET->m_bContryID != TCONTRY_N )
	{
		...
		else
		{
			...
			if( m_pMainChar->m_bContryID == TCONTRY_B ||
				((CTClientNpc*)m_pTARGET)->m_bNPCType == TNPC_AUCTION )
			{
				bAlliance = TRUE;
			}
		}
...
int CTClientGame::OnGM_NPC_GUILD_NEW()
{
	if( !m_pTARGET || m_pTARGET->m_bType != OT_NPC || (
		...
		m_pMainChar->IsDead() || m_pMainChar->m_bGhost)) ||
		m_pMainChar->IsBlock() || m_pMainChar->IsDown() ||
		!m_pMainChar->IsAvailableNPC(m_pTARGET) )
	{
		return TERR_NONE;
	}
...
int CTClientGame::OnGM_PORTAL_REQ_DIRECT()
{
	if( !m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;

	WORD wPortalID = LOWORD(m_dwCmdParam);
...
int CTClientGame::OnGM_PORTAL_REQ()
{
	...
	if(pInfo)
	{
		...
		{
			if( pTPORTAL->m_wPortalRegionID == 100 )
			{
				m_pMainWnd->MessageBoxYesNo(
					TSTR_MEETING_CAUTION_FUCKING,
					TSTR_YES,
					TSTR_NO,
					TCOMMAND(GM_PORTAL_REQ_DIRECT, MAKELONG(pTPORTAL->m_wPortalID, (WORD) m_pTARGET->m_dwID)),
					GM_CLOSE_MSGBOX,
					GM_CLOSE_MSGBOX );
			}
			else
...
#ifdef MAGICITEMSHOP
int CTClientGame::OnGM_MAGICITEM_BUY()
{
	CTMagicItemShopDlg* pMagicDlg = (CTMagicItemShopDlg*) m_vTFRAME[ TFRAME_MAGICITEM_SHOP ];
	CTCtrlList* pList = pMagicDlg->GetList();
	CTMagicShopItem* pItem = static_cast< CTMagicShopItem* >( pList->GetSelectItem() );
...
int CTClientGame::OnGM_WARN_UPITEM_SELL_OK()
{
	...
	TFrame *pFRAME = m_pMainWnd->m_TNet.FindFrame(ID_FRAME_LOGIN_NEW);
...
#ifdef MAGICITEMSHOP
int CTClientGame::OnGM_MAGICITEM_DO_BUY()
{
	if( !m_pTARGET )
		return TERR_NONE;

	CTMagicItemShopDlg* pMagicShop = (CTMagicItemShopDlg*) GetFrame( TFRAME_MAGICITEM_SHOP );
	CTCtrlList* pList = pMagicShop->GetList();
	CTMagicShopItem* pItem = static_cast<CTMagicShopItem*>( pList->GetSelectItem() );

	if( pItem && m_pSESSION )
	{
		m_pSESSION->SendCS_MAGICITEMBUY_REQ(
			(WORD)m_pTARGET->m_dwID,
			pItem->m_dwMagicItemID,
			1,
			INVEN_NULL,
			INVALID_SLOT );
	}
...
int CTClientGame::OnGM_QUEST_MAPOPEN()
{
	if ( m_vTFRAME[TFRAME_QUEST]->IsVisible() )
	{
		CTNewQuestDlg *pDLG = (CTNewQuestDlg *) m_vTFRAME[ TFRAME_QUEST ];
		pDLG->OnQuestMapOpen();
	}
	else if (m_vTFRAME[TFRAME_NPCDIALOG]->IsVisible() )
	{
		CTNPCTalkDlg* pDLG = (CTNPCTalkDlg *) m_vTFRAME[ TFRAME_NPCDIALOG ];
		pDLG->OnQuestMapOpen();
	}
...
int CTClientGame::OnGM_QUEST_MAPCLOSE()
{
	if ( m_vTFRAME[TFRAME_QUEST]->IsVisible() )
	{
		CTNewQuestDlg *pDLG = (CTNewQuestDlg *) m_vTFRAME[ TFRAME_QUEST ];
		pDLG->OnQuestMapClose();
	}
	else if ( m_vTFRAME[TFRAME_NPCDIALOG]->IsVisible() )
	{
		CTNPCTalkDlg* pDLG = (CTNPCTalkDlg *) m_vTFRAME[ TFRAME_NPCDIALOG ];
...
int CTClientGame::OnGM_GOTO_THUMB()
{
	...
	if(pTREGION)
	{
		...
		{
			D3DXVECTOR3 vTHUMB = pTREGION->GetThumbPosition( m_pMainChar->m_bCamp - 1 );
			m_pMainChar->m_vWorld._41 = vTHUMB.x;
			m_pMainChar->m_vWorld._42 = vTHUMB.y;
			m_pMainChar->m_vWorld._43 = vTHUMB.z;
		}
		else
		{
			D3DXVECTOR3 vTHUMB = pTREGION->GetThumbPosition( m_pMainChar->GetWarCountry() );
			m_pMainChar->m_vWorld._41 = vTHUMB.x;
			m_pMainChar->m_vWorld._42 = vTHUMB.y;
			m_pMainChar->m_vWorld._43 = vTHUMB.z;
		}
...
{
	if( m_pMainChar->m_dwTacticsID == 0 )
	{
		CString strWARN;
...
{
	m_pMainWnd->CloseMessageBox();
	CTGuildCommander::GetInstance()->RequestGuildLeave();
	return TERR_NONE;
}

int CTClientGame::OnGM_GUILD_TACTICS_SELF_QUIT()
{
	m_pMainWnd->CloseMessageBox();
	...
	{
		if( m_pMainChar->m_dwTacticsID != 0 )
			m_pSESSION->SendCS_GUILDTACTICSKICKOUT_REQ( m_pMainChar->m_dwID );
	}
	return TERR_NONE;
}
...
int CTClientGame::OnGM_GUILD_DONATE_PVPPOINT()
{
	CTGuildExpDonDlg* pDlg = static_cast<CTGuildExpDonDlg*>( m_vTFRAME[TFRAME_DONATE_EXP] );
	
	if( pDlg->IsVisible() )
		DisableUI(TFRAME_DONATE_EXP);		
	else if( pDlg->InitValue(0) )
		EnableUI(TFRAME_DONATE_EXP);
...
int CTClientGame::OnGM_WRAN_DONATE_EXP()
{
	...
	if( pDlg->IsVisible() )
	{
		...
		if( pDlg->m_bMode )
			...
			strMSG = CTChart::Format( TSTR_WRAN_DONATE_PVP, dwEXP);
...
int CTClientGame::OnGM_DO_DONATE_EXP()
{
	...
	{
		DWORD dwPVP = pDlg->GetDonationExp();

		CTGuildCommander::GetInstance()
			->RequestGuildDonationPVP(dwPVP);
	}
...
int CTClientGame::OnGM_CHANGE_CHANNEL()
{
	if( m_pChannelPOPUP->IsVisible() )
	{
		...
		if( nChNum >= m_pMainWnd->m_pTGROUP->m_bSelectedChannelIndex )
			++nChNum;

		if( nChNum < (INT) m_pMainWnd->m_pTGROUP->m_vChannel.size()  )
			DoChangeChannel( m_pMainWnd->m_pTGROUP->m_vChannel[ nChNum ]->bID );
	...
	else
	{
		...
		for( size_t i=0 ; i < m_pMainWnd->m_pTGROUP->m_vChannel.size() ; ++i )
		{
			if( m_pMainWnd->m_pTGROUP->m_bSelectedChannelIndex == (BYTE)i )
				continue;

			m_pChannelPOPUP->AddMENU(
				m_pMainWnd->m_pTGROUP->m_vChannel[ i ]->strNAME,
				GM_CHANGE_CHANNEL);
		}
...
int CTClientGame::OnGM_AUCTION_REGIST_TAB()
{
	...
	if( m_pTARGET->m_bContryID != m_pMainChar->m_bContryID )
	{
		CString strMSG;

		switch(m_pTARGET->m_bContryID)
		{
			case TCONTRY_D:
				strMSG = CTChart::LoadString( TSTR_AUCTION_ONLY_D );
				break;
			case TCONTRY_C:
				strMSG = CTChart::LoadString( TSTR_AUCTION_ONLY_C );
				break;
			case TCONTRY_B:
				strMSG = CTChart::LoadString( TSTR_AUCTION_ONLY_B );
				break;
		}
...
int CTClientGame::OnGM_GUILD_OFFER_BBS_DO_DELETE()
{
	...
	if( m_pMainChar->m_dwTacticsID != 0 ||
		m_pMainChar->m_bGuildDuty != GUILD_DUTY_VICECHIEF &&
...
int CTClientGame::OnGM_GMP_TACTICS_SELF_KICK()
{
	CTCommunityDlg* pDlg = static_cast<CTCommunityDlg*>( m_vTFRAME[TFRAME_COMMUNITY] );
	CTGuildMemberFrame* pFrame = static_cast<CTGuildMemberFrame*>( pDlg->GetInnerFrame(TCOMMUNITY_FRAME_GUILDMEMBER) );
	return pFrame->OnGM_GMP_TACTICS_SELF_KICK();
}
...
{
	if( m_pMainChar->m_bGuildDuty == GUILD_DUTY_CHIEF && m_pMainChar->m_dwTacticsID == 0 )
		EnableUI( TFRAME_GUILDMARK );
	else
		m_pMainWnd->MessageBoxOK(
...
int CTClientGame::OnGM_EXIT_TUTORIAL()
{
	m_pMainWnd->MessageBoxYesNo(
...
int CTClientGame::OnGM_CLICKED_STOP_TUTORIAL()
{
	...
	DisableUI( TFRAME_WELCOME );
	DisableUI( TFRAME_HELP );
	...
	((CTGaugePannel *) m_vTFRAME[TFRAME_GAUGE])->DettachExitTutorial();
...
int CTClientGame::OnGM_CLICKED_SIEGE_ALARM_DO()
{
	if(m_pSESSION)
		...
		switch( m_pMainChar->GetWarCountry() )
		{
...
int CTClientGame::OnGM_SKIP_SCENE()
{
	...
	{
		switch( m_vTSCENE_PLAYER.m_nCurScene )
		{
		case TSCENE_SCRIPT_DEFUGEL_ENTER:
			m_vTSCENE_PLAYER.StartScene(
				m_pDevice,
				m_pRES,
				TSCENE_SCRIPT_DEFUGEL_SKIP);
			break;

		case TSCENE_SCRIPT_CRAXION_ENTER:
			m_vTSCENE_PLAYER.StartScene(
				m_pDevice,
				m_pRES,
				TSCENE_SCRIPT_CRAXION_SKIP);
			break;

		case TSCENE_SCRIPT_BROIR_ENTER:
			m_vTSCENE_PLAYER.StartScene(
				m_pDevice,
				m_pRES,
				TSCENE_SCRIPT_BROIR_SKIP);
			break;

		case TSCENE_SCRIPT_PEACE_ENTER:
			m_vTSCENE_PLAYER.StartScene(
				m_pDevice,
				m_pRES,
				TSCENE_SCRIPT_PEACE_SKIP);
			break;
		}
	}
	return TERR_NONE;
...
int CTClientGame::OnGM_SELECT_DO_AID()
{
	m_pMainWnd->CloseMessageBox(TRUE);

	if( m_pSESSION )
	{
		m_pSESSION->SendCS_CHANGECOUNTRY_REQ(
			IK_AIDCOUNTRY,
			(BYTE) m_dwCmdParam,
			0, 0 );
	}
...
int CTClientGame::OnGM_SELECT_AID_D()
{
	DisableUI( TFRAME_AID_COUNTRY );

	if( m_pMainChar->m_bAidCountryID == TCONTRY_D )
	{
		m_pMainWnd->MessageBoxOK(
			TSTR_NATIONALITY_ALREADY_AID_D,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
			0,
			FALSE);
	...
	{
		m_pMainWnd->MessageBoxYesNo(
			TSTR_NATIONALITY_REALLY_AID_D,
			TSTR_YES,
			TSTR_NO,
			TCOMMAND(GM_SELECT_DO_AID, TCONTRY_D),
			GM_CLOSE_MSGBOX,
			GM_CLOSE_MSGBOX);
	}
...
int CTClientGame::OnGM_SELECT_AID_C()
{
	DisableUI( TFRAME_AID_COUNTRY );

	if( m_pMainChar->m_bAidCountryID == TCONTRY_C )
	{
		m_pMainWnd->MessageBoxOK(
			TSTR_NATIONALITY_ALREADY_AID_C,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
			0,
			FALSE);
	...
	{
		m_pMainWnd->MessageBoxYesNo(
			TSTR_NATIONALITY_REALLY_AID_C,
			TSTR_YES,
			TSTR_NO,
			TCOMMAND(GM_SELECT_DO_AID, TCONTRY_C),
			GM_CLOSE_MSGBOX,
			GM_CLOSE_MSGBOX);
	}
...
int CTClientGame::OnGM_NPC_MEETING()
{
	...
	m_pMainWnd->MessageBox(
		TSTR_MEETING_ENTER_NAME,
		0, 0,
		TSTR_YES,
		TSTR_NO,
		ID_FRAME_2BTNMSGBOX,
		GM_INVITE_MEETING_ROOM,
		GM_CLOSE_MSGBOX_WITH_RESETCONMODE,
		GM_CLOSE_MSGBOX_WITH_RESETCONMODE,
		TRUE,
		TRUE);
...
int CTClientGame::OnGM_INVITE_MEETING_ROOM()
{
	if( !m_pSESSION || m_pMainWnd->m_MessageBox.GetParent() != this )
		return TERR_NONE;
	...
	if( m_pSESSION )
		m_pSESSION->SendCS_MEETINGROOM_REQ( 0, 0, strTARGET );

	return TERR_NONE;
}

int CTClientGame::OnGM_NPC_MERCENARY()
{
	if( NULL == m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	if( m_pSESSION )
	{
		m_pSESSION->SendCS_WARCOUNTRYBALANCE_REQ();
	}
	return TERR_NONE;
}

int CTClientGame::OnGM_NPC_NATIONALITY1()
{
	EnableUI( TFRAME_SELECT_COUNTRY );
	return TERR_NONE;
}

int CTClientGame::OnGM_NPC_NATIONALITY2()
{
	if( NULL == m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	if( fDist > TARGETUI_DIST )
	{
		ReleaseMoveType();
		m_bMoveType = TMAINMOVE_TARGET;
		m_dwMoveGM = GM_NPC_NATIONALITY2;
		m_bAutoRun = FALSE;
		m_fBOUND = POPUP_DIST;
		return TERR_NONE;
	}

	if( m_pMainChar->m_bContryID == TCONTRY_C ||
		m_pMainChar->m_bContryID == TCONTRY_D )
	{
		if( m_pMainChar->m_bLevel >= 80 )
		{
			m_pMainWnd->MessageBoxYesNo(
				TSTR_NATIONALITY_CAUTION,
				TSTR_YES,
				TSTR_NO,
				GM_DO_CHANGE_TO_BROIR,
				GM_CLOSE_MSGBOX );
		}
		else
		{
			m_pMainWnd->MessageBoxOK(
				TSTR_NATIONALITY_LEVEL,
				TSTR_OK,
				GM_CLOSE_MSGBOX,
				0 );
		}
	}
	else if( m_pMainChar->m_bContryID == TCONTRY_B )
	{
		m_pMainWnd->MessageBoxOK(
			TSTR_NATIONALITY_BROIR,
			TSTR_OK,
			GM_CLOSE_MSGBOX,
			0 );
	}
...
int CTClientGame::OnGM_DO_CHANGE_TO_BROIR()
{
	m_pMainWnd->CloseMessageBox();
...
int CTClientGame::OnGM_SELECT_MY_COUNTRY()
{
	if( NULL == m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	if( fDist > TARGETUI_DIST )
	{
		ReleaseMoveType();
		m_bMoveType = TMAINMOVE_TARGET;
		m_dwMoveGM = GM_SELECT_MY_COUNTRY;
		m_bAutoRun = FALSE;
		m_fBOUND = POPUP_DIST;
		return TERR_NONE;
	}

	if( m_pSESSION && m_pTARGET )
	{
		if( m_pMainChar->m_bContryID == TCONTRY_PEACE )
		{
			if( m_pMainChar->m_bLevel >= CHOICE_COUNTRY_LEVEL )
			{
				EnableUI( TFRAME_SELECT_COUNTRY );
			}
			else
...
int CTClientGame::OnGM_REALLY_SELECT_COUNTRY()
{
	m_pMainWnd->CloseMessageBox(TRUE);

	m_pMainWnd->MessageBoxYesNo(
		TSTR_NATIONALITY_ASK_REALLY,
		TSTR_YES,
		TSTR_NO,
		TCOMMAND(GM_DO_SELECT_COUNTRY, m_dwCmdParam),
		GM_CLOSE_MSGBOX );

	return TERR_NONE;
}

int CTClientGame::OnGM_SELECT_MY_COUNTRY_D()
{
	if( NULL == m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	if( fDist > TARGETUI_DIST )
	{
		ReleaseMoveType();
		m_bMoveType = TMAINMOVE_TARGET;
		m_dwMoveGM = GM_SELECT_MY_COUNTRY_D;
		m_bAutoRun = FALSE;
		m_fBOUND = POPUP_DIST;
		return TERR_NONE;
	}
	...
	if( m_pMainChar->m_bContryID == TCONTRY_PEACE )
	{
		if( m_pMainChar->m_bLevel >= CHOICE_COUNTRY_LEVEL )
		{
			...
			m_pMainWnd->MessageBoxYesNo(
				...
				TSTR_YES,
				TSTR_NO,
				TCOMMAND(GM_REALLY_SELECT_COUNTRY, TCONTRY_D),
				GM_CLOSE_MSGBOX );
		}
		else
		{
...
int CTClientGame::OnGM_SELECT_MY_COUNTRY_C()
{
	if( NULL == m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	if( fDist > TARGETUI_DIST )
	{
		ReleaseMoveType();
		m_bMoveType = TMAINMOVE_TARGET;
		m_dwMoveGM = GM_SELECT_MY_COUNTRY_C;
		m_bAutoRun = FALSE;
		m_fBOUND = POPUP_DIST;
		return TERR_NONE;
	}
	...
	if( m_pMainChar->m_bContryID == TCONTRY_PEACE )
	{
		if( m_pMainChar->m_bLevel >= CHOICE_COUNTRY_LEVEL )
		{
			...
			m_pMainWnd->MessageBoxYesNo(
				...
				TSTR_YES,
				TSTR_NO,
				TCOMMAND(GM_REALLY_SELECT_COUNTRY, TCONTRY_C),
				GM_CLOSE_MSGBOX );
		}
		else
		{
...
int CTClientGame::OnGM_DO_SELECT_COUNTRY()
{
	DisableUI( TFRAME_SELECT_COUNTRY );
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION &&
		((TCONTRY_TYPE)m_dwCmdParam == TCONTRY_D || (TCONTRY_TYPE)m_dwCmdParam == TCONTRY_C) )
	{
...
int CTClientGame::OnGM_ANSWER_METTING_OK()
{
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION )
		m_pSESSION->SendCS_MEETINGROOM_REQ( 1, ASK_YES, m_strMeetingInviter );

	return TERR_NONE;
}

int CTClientGame::OnGM_ANSWER_MEETING_NO()
{
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION )
		m_pSESSION->SendCS_MEETINGROOM_REQ( 1, ASK_NO, m_strMeetingInviter );

	return TERR_NONE;
}

int CTClientGame::OnGM_ANSWER_MEETING_IGNORE()
{
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION )
		m_pSESSION->SendCS_MEETINGROOM_REQ( 1, ASK_BUSY, m_strMeetingInviter );

	return TERR_NONE;
}

int CTClientGame::OnGM_CHALLENGE_JOIN_DO()
{
	m_pMainWnd->CloseMessageBox();

	CTChallengeWaiting* pDLG = static_cast<CTChallengeWaiting*>(m_vTFRAME[ TFRAME_CHALLENGE_WAITING ] );
	if( m_pSESSION )
	{
		m_pSESSION->SendCS_ARENA_REQ(
			0,
			(WORD) pDLG->m_dwNPC,
			0 );
	}
...
int CTClientGame::OnGM_CHALLENGE_JOIN()
{
	if( NULL == m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;

	m_pMainWnd->MessageBoxYesNo(
		TSTR_CHALLENGE_ASK_GO,
		TSTR_YES,
		TSTR_NO,
		GM_CHALLENGE_JOIN_DO,
		GM_CLOSE_MSGBOX );

	DisableUI( TFRAME_CHALLENGE_WAITING );

	return TERR_NONE;
}

int CTClientGame::OnGM_CHALLENGE_GIVEUP_DO()
{
	m_pMainWnd->CloseMessageBox();
	...
	if( m_pSESSION )
	{
		m_pSESSION->SendCS_ARENA_REQ(
			1,
			0,
			m_pMainChar->m_wChallengeArenaID );
	}
	return TERR_NONE;
}

int CTClientGame::OnGM_CHALLENGE_GIVEUP()
{
	DisableUI( TFRAME_CHALLENGE_WAITING );
	...
	{
		m_pMainWnd->MessageBoxYesNo(
			TSTR_CHALLENGE_ASK_GIVEUP,
			TSTR_YES,
			TSTR_NO,
			GM_CHALLENGE_GIVEUP_DO,
			GM_CLOSE_MSGBOX );
	}
...
int CTClientGame::OnGM_CHALLENGE_EVENT()
{
	return TERR_NONE;
}

int CTClientGame::OnGM_NPC_DUEL3()
{
	if( NULL ==  m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	{
		CTChallengeWaiting* pDLG = static_cast<CTChallengeWaiting*>(m_vTFRAME[ TFRAME_CHALLENGE_WAITING ] );
		pDLG->m_dwNPC = m_pTARGET->m_dwID;
		EnableUI( TFRAME_CHALLENGE_WAITING );
	}

	return TERR_NONE;
}

int CTClientGame::OnGM_NPC_DUEL7()
{
	if( NULL ==  m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
...
int CTClientGame::OnGM_SELECT_AID_GIVEUP_DO()
{
	m_pMainWnd->CloseMessageBox();
...
int CTClientGame::OnGM_SELECT_AID_GIVEUP()
{
	m_pMainWnd->MessageBoxYesNo(
		TSTR_NATIONALITY_ASK_GIVEUP_AID,
		TSTR_YES,
		TSTR_NO,
		GM_SELECT_AID_GIVEUP_DO,
		GM_CLOSE_MSGBOX );

	return TERR_NONE;
}

int CTClientGame::OnGM_CHANGE_ORG_COUNTRY()
{
	m_pMainWnd->CloseMessageBox();
...
#ifdef ADD_SKYGARDEN
int CTClientGame::OnGM_ASSISTANT_REQ()
{
	if( m_pTARGET && m_pTARGET->m_bType == OT_PC && m_pSESSION )
	{
		CTClientChar* pTargetChar = static_cast<CTClientChar*>( m_pTARGET );
		m_pSESSION->SendCS_ASSISTANT_REQ(pTargetChar->m_strNAME);
	}

	return TERR_NONE;
}

int CTClientGame::OnGM_ASSISTANT_OK()
{
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION )
		m_pSESSION->SendCS_ASSISTANTANSWER_REQ(ASK_YES, m_strAssistantInvitor);
	m_strAssistantInvitor.Empty();
	return TERR_NONE;
}

int CTClientGame::OnGM_ASSISTANT_NO()
{
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION )
		m_pSESSION->SendCS_ASSISTANTANSWER_REQ(ASK_NO, m_strAssistantInvitor);
	m_strAssistantInvitor.Empty();
	return TERR_NONE;
}

int CTClientGame::OnGM_ASSISTANT_BUSY()
{
	m_pMainWnd->CloseMessageBox();

	if( m_pSESSION )
		m_pSESSION->SendCS_ASSISTANTANSWER_REQ(ASK_BUSY, m_strAssistantInvitor);
	m_strAssistantInvitor.Empty();
	return TERR_NONE;
}

int CTClientGame::OnGM_ASSISTANT_KICK()
{
	if( m_pTARGET && m_pTARGET->m_bType == OT_PC && m_pSESSION )
	{
		CTClientChar* pTargetChar = static_cast<CTClientChar*>( m_pTARGET );
		m_pSESSION->SendCS_ASSISTANTDEL_REQ(pTargetChar->m_dwID);
	}

	return TERR_NONE;
}

int CTClientGame::OnGM_ASSISTANT_SELFKICK()
{
	if( m_pMainChar->m_wAssistantID != 0 && m_pSESSION )
	{
		m_pSESSION->SendCS_ASSISTANTDEL_REQ(m_pMainChar->m_dwID);
	}
...
int CTClientGame::OnGM_SELECT_COUNTRY_DLG()
{
	CTSelectCountry* pDlg = (CTSelectCountry*) m_vTFRAME[ TFRAME_SELECT_COUNTRY ];
	if( (BYTE)-1 != pDlg->m_bSelectedCountry )
	{
		m_dwCmdParam = pDlg->m_bSelectedCountry;
		OnGM_DO_SELECT_COUNTRY();
	}
	return TERR_NONE;
}

int CTClientGame::OnGM_NODE_TELEPORT_YES()
{
	m_pMainWnd->CloseMessageBox();

	if( !m_pTAUTOPORTAL || !m_pTARGET || m_pTARGET->m_bType != OT_NPC )
		return TERR_NONE;
	...
	{
		CTPortalShopItem *pTITEM = (CTPortalShopItem *) pTLIST->GetItem(i);

		if( pTITEM && pTITEM->m_bItemType == CTPortalShopItem::TPORTALSHOPITEM_TYPE_PORTAL &&
			pTITEM->m_pTPortal && m_pTAUTOPORTAL->m_pTPORTAL &&
			pTITEM->m_pTPortal->m_wPortalID == m_pTAUTOPORTAL->m_pTPORTAL->m_wPortalID )
		{
			pTLIST->Select(pTITEM->m_nIndex);
			return OnGM_PORTAL_REQ();
		}
	}
...
int CTClientGame::OnGM_NODE_TELEPORT_NO()
{
	m_pMainWnd->CloseMessageBox();
	ReleaseMoveType();
...
int CTClientGame::OnGM_TUTORIAL_START()
{
	DisableUI(TFRAME_WELCOME);
	return TERR_NONE;
}

int CTClientGame::OnGM_TUTORIAL_SKIP()
{
	DisableUI(TFRAME_WELCOME);
	OnGM_EXIT_TUTORIAL();
	return TERR_NONE;

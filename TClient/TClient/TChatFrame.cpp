	  ...
	  m_dwChatTargetID(0), m_pChatEditBack(NULL), m_pChatTabBack(NULL), m_dwNotifyTime(0)
{
	...
	for( INT i=0; i<TCHAT_SET_COUNT; ++i)
	{
		...
		m_vLists[i]->m_bBasisPoint = offset.m_bBasisPoint;
		m_vLists[i]->m_vCompOffset = offset.m_vOffset;
...
TListItem* CTChatFrame::HitTestChatList( CPoint pt )
{
	for( INT i=0; i<TCHAT_SET_COUNT; ++i)
	{
		CTChatList* pList = m_vLists[i];

		if( pList->IsOuter() || m_pSelectTab->GetChatList() == pList )
		{
			if( pList->HitRect( pt ) )
			{
				int j = pList->m_nTop;

				VECTOR_TCOMP_ARRAY::iterator itor = pList->m_itemholders.begin();
				VECTOR_TCOMP_ARRAY::iterator end = pList->m_itemholders.end();

				size_t nItemSize = pList->m_items.size();
				for( ; itor != end; ++itor, ++j )
				{
					TCOMP_ARRAY* pHolder = (*itor);
					if( !pHolder )
						continue;

					if( j >= nItemSize || (j - pList->m_nTop) >= pList->m_nItemPerPage )
						continue;

					TComponent* pHolderCol = pHolder->at( 0 );
					if( pHolderCol && pHolderCol->HitTest( pt ) )
					{
						MAP_LISTITEM* pItems = pList->m_items[j];
						MAP_LISTITEM::iterator finder = pItems->find( 0 );

...
BYTE CTClientChar::IsAlliance( CTClientObjBase *pTARGET)
{
	...
	if( pGame->m_bChallengeGame )
	{
		...
		if( pHostTarget )
		{
			if( m_bChallengeTeam != TTOURNAMENT_TEAM_NONE &&
				pHostTarget->m_bChallengeTeam != TTOURNAMENT_TEAM_NONE &&
				pHostTarget->m_bChallengeTeam != m_bChallengeTeam  )
			{
				return FALSE;
			}
		}
...
bool CTClientChar::IsAvailableNPC( CTClientObjBase* pTARGET )
{
	if( pTARGET->m_bType != OT_NPC )
		return false;

	if( m_bContryID == TCONTRY_B && pTARGET->m_bContryID == TCONTRY_B )
		return true;

	if( IsAlliance( pTARGET ) )
		return true;
	else if( pTARGET->m_bContryID == TCONTRY_N )
		return true;

	return false;
}

INT CTClientChar::GetRandomAniIndex(
	DWORD dwActionID,
	INT nCount)
{

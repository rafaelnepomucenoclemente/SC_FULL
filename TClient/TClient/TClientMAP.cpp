...
#ifdef TEST_MODE
		if( pINST->m_pTNPC == NULL )
		{
			FILE* fp = fopen("npc_error.txt", "at");
			if( fp )
			{
				fseek(fp, 0, SEEK_END);
				fprintf(fp,"NPC ID - %d를 찾을 수 없습니다.\n", pINST->m_dwID );
				fclose(fp);
			}
			return;
		}
...
int CTClientMAP::GetTDRAWCELL( FLOAT fPosX,
							   FLOAT fPosZ)
{
	int nLocalX = INT(fPosX / TOBJCELL_LENGTH) - m_nTCellX;
	int nLocalZ = INT(fPosZ / TOBJCELL_LENGTH) - m_nTCellZ;

	if( 0 <= nLocalX && nLocalX < TDRAWCELL_WIDTH &&
		0 <= nLocalZ && nLocalZ < TDRAWCELL_WIDTH )
	{
		int nIndex = nLocalZ * TDRAWCELL_WIDTH + nLocalX;

		if( m_vTCELL[nIndex] != 0xFFFF && m_vLOCK[m_vTCELL[nIndex]] )
			return m_vTCELL[nIndex];
	}
		...
		bRESULT )
	{
		...
		for( i=0; i<9; i++)
			if( m_pMAP->m_bUNIT[i] != 0xFF && !IsValidLoadUNIT( m_pTLOADJOINT, i) )
			{
				...
				m_vTNODE[m_pMAP->m_bUNIT[i]].ReleaseTPATH();
...
CTClientObjBase *CTClientMAP::GetTNPCINST( DWORD dwNpcID)
{
	CTClientObjBase *pTOBJ = FindTNPC(dwNpcID);

	if(!pTOBJ)
	{
		MAPOBJECT::iterator finder = m_mapTPortal.find(dwNpcID);

		if( finder != m_mapTPortal.end() )
			pTOBJ = (CTClientObjBase *) (*finder).second;
	}

	return pTOBJ;
}

LPTNPC CTClientMAP::GetTNPC( WORD wNpcID)
{
	MAPNPC::iterator finder = m_mapTLOADEDNPC.find(wNpcID);

	if( finder != m_mapTLOADEDNPC.end() )
...
void CTClientMAP::ClearTNPC( LPVTNPC pTNPC)
{
	while(!pTNPC->empty())
	{
		LPTNPC pNpc = pTNPC->back();

		MAPNPC::iterator finder = m_mapTLOADEDNPC.find(WORD(pNpc->m_dwID));
		if( finder != m_mapTLOADEDNPC.end() )
			m_mapTLOADEDNPC.erase(finder);
...
void CTClientMAP::LoadTPATH( CTClientPath *pTPATH,
							 WORD wUnitID)
{
	...
	strFile.Format(
		_T(".\\Node\\TNODE%04X%02X%02X.pnd"),
		m_wMapID,
		bUnitX,
		bUnitZ);

	pTPATH->LoadTPATH(strFile);
}
								...
								FLOAT fPosZ)
{
	...
	{
		MAPDWORD::iterator finder = CTChart::m_mapTSTEPSND.find(MAKELONG( wTileID, WORD(m_wMapID)));
								  ...
								  LPTLOADOBJ pTINFO)
{
	...
	for( i=0; i<INT(pTINFO->m_vUnitID.size()); i++)
	{
		...
		LoadTPATH( &vTNODE[bINDEX], wUnitID);
	...
	if( m_pMAP && !pTAPP->m_bCancelLoad )
		for( i=0; i<INT(pTINFO->m_vUnitID.size()); i++)
		{
			...
			m_vTNODE[bINDEX].LoadTPATH(&vTNODE[bINDEX]);
			m_vTNODE[bINDEX].m_fLengthX = fUnitLength;
			m_vTNODE[bINDEX].m_fLengthZ = fUnitLength;
			m_vTNODE[bINDEX].m_fBaseX = fUnitX;
			m_vTNODE[bINDEX].m_fBaseZ = fUnitZ;
			...
			while(!vTNPC[bINDEX].empty())
			{
				...
				m_mapTLOADEDNPC.insert( MAPNPC::value_type( WORD(pTNPC->m_dwID), pTNPC));
			}

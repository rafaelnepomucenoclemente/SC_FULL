...
m_pPopup(NULL)
{
	...
	m_pColumnC->SetItemString( i, 3, CTChart::LoadString( TSTR_GUILD_BELONG ) );
	...
	m_pColumnC->SetItemString( i, 1, CTChart::LoadString( TSTR_GUILD_JOB ) );
	m_pColumnC->SetItemString( i, 2, CTChart::LoadString( TSTR_GUILD_LEVEL ) );
	m_pColumnC->SetItemString( i, 3, CTChart::LoadString( TSTR_GUILD_TACTICS_POINT1 ) );
	m_pColumnC->SetItemString( i, 4, CTChart::LoadString( TSTR_GUILD_TACTICS_POINT2 ) );
					...
					case RINFO_SERVICE:
						{
							m_pListC->SetItemString( nLine, 3,
								CTChart::LoadString( (TSTRING) CTClientGame::m_vTCOUNTRYSTR[ itMEMBER->m_bWarCountry ] ) );
...
int CTGuildMemberFrame::OnGM_GMP_TACTICS_SELF_KICK()
{
	CancelPopup();
